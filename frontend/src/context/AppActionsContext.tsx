...
export function AppActionsProvider({ children }: { children: ReactNode }) {
  ...
  const routeProfileId = location.pathname.match(
    /^\/(?:profile|roadmap|report|coach|fear-transformer|recommendations|assessment|career-compatibility|career-compare|learning)\/([^/]+)/
  )?.[1] || location.pathname.match(/^\/workspace\/(?:profiles\/)?([^/]+)/)?.[1];
  ...
  const executeVoiceCommand = useCallback((rawCommand: string): VoiceCommandResult => {
    ...
    if (["open learning paths", "open learning path", "deschide traseele de invatare"].includes(command)) return recognized("Open Learning Path", () => navigate(`/workspace/${activeProfileId}/learning`), "Opening your learning path.");
