        ...
        <main className="space-y-5">
            ...
            {coach.error ? (
              <div role="alert" className="mt-4 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {coach.canRetryLastMessage ? <button type="button" onClick={() => void coach.retryLastMessage()} className="organic-button-secondary">Retry message</button> : null}
                  ...
                  <button
                    type="button"
                      onClick={() => {
                        ...
                        else void coach.liveVoice.approveVoiceConsentAndStart();
                      }}
          ...
          <section className="glass-card p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {latest?.sourcesUsed?.map((source, index) => <Link to="/knowledge-base" key={`${source.id}:${source.section_title}:${index}`} className="organic-chip text-xs">{source.document_name} - {source.section_title}</Link>)}
        ...
        <aside className="space-y-4">
          <section className="organic-card-dark flex max-h-[650px] flex-col p-5 text-white">
            ...
            <div className="mt-4 flex-1 space-y-3 overflow-y-auto">
              ...
              {coach.isSending ? <p aria-live="polite" className="mr-8 rounded-2xl bg-white/10 p-3 text-sm text-white/75">OrganicAI Coach is preparing your response…</p> : null}
            </div>
            <div className="mt-4 flex gap-2">
              ...
              />
              <button type="button" aria-label="Send message" onClick={() => send(input)} disabled={coach.isSending} className="organic-icon-orb h-11 w-11" title={coach.isSending ? "Sending message" : "Send message"}><Send size={17} /></button>
            </div>
          </section>
          <section className="glass-card p-5">
