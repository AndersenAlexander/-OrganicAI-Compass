...
import { loginFailureMessage } from "../api/authApi";
...
export function LoginPage() {
  const { login, loginDemo, isAuthenticated, isDemo, isLoading } = useAuth();
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(loginFailureMessage(error));
    } finally {
      setIsSubmitting(false);
    }
  }
  async function handleDemo(){setError(null);setDemoLoading(true);try{const profileId=await loginDemo();navigate(`/profile/${profileId}`)}catch(error){setError(demoLoginFailureMessage(error))}finally{setDemoLoading(false)}}
      ...
      <Card className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" disabled={isSubmitting || isLoading}>
            <LogIn size={18} /> {isSubmitting || isLoading ? "Logging in..." : "Log in"}
          </Button>
        </form>
