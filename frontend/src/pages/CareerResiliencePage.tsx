...
export function CareerResiliencePage() {
  ...
  async function createSession(templateId: string, addToRoadmap = false) {
    const created = await createCareerExperiment(profileId, {
      experiment_template_id: templateId,
      mode: "guided",
      user_confirmed: true,
      add_to_roadmap: addToRoadmap,
    });
    setStatus(addToRoadmap ? "Experiment planned and added to My Roadmap after confirmation." : "Experiment planned. My Roadmap was not changed.");
  ...
  async function handleReviewAndEvaluate(session: CareerExperimentSession) {
    ...
    await recalibrateCareer(profileId, evaluated.result?.id);
    setSessionDetail(evaluated);
    setStatus("Evidence Passport updated and career direction recalibrated.");
    await refresh();
  }
      ...
      {section === "dashboard" ? (
        <>
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <SignalCard title="Active experiments" value={String(dashboard?.active_experiments.length ?? 0)} icon={<FlaskConical size={18} />} />
                ...
                {(dashboard?.career_hypotheses || []).map((hypothesis) => (
                  <div key={hypothesis.id} className="rounded-2xl border border-[color:var(--border-soft)] p-4">
                    ...
                    <button
                      ...
                      onClick={() => void createCareerExperiment(profileId, { career_match_id: hypothesis.career_match_id, mode: "guided", user_confirmed: true }).then((created) => navigate(`${base}/experiments/${created.id}`))}
                    >
            ...
            {selectedSession ? (
              <div className="mt-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button type="button" className="organic-button-secondary" onClick={() => void createSession(selectedSession.experiment_template_id, true)}>
                    <BriefcaseBusiness size={16} /> Add to Roadmap
                  </button>
                </div>
                <div className="grid gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button type="button" className="organic-button" disabled={!selectedSession.submission && selectedSession.status !== "submitted" && selectedSession.status !== "needs_review"} onClick={() => void handleReviewAndEvaluate(selectedSession)}>
                      <BadgeCheck size={16} /> Review evidence
                    </button>
                  </div>
                </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            {(passport?.skills || []).map((skill) => (
              <article key={skill.skill_id} className="rounded-2xl border border-[color:var(--border-soft)] p-4">
                ...
                <ul className="mt-3 list-disc space-y-1 pl-5 text-sm theme-muted">
                  {skill.evidence_sources.slice(0, 3).map((source) => <li key={source.id}>{source.title} - {source.strength}</li>)}
