...
import { createCareerExperiment } from "../api/careerResilienceApi";
...
import type { CareerExperimentRecommendation } from "../types/careerResilience";
import { useAppActions } from "../hooks/useAppActions";
import { displayHypothesisDimensions } from "../lib/humanDiscoveryJourney";
...
function MatchCard({
  ...
  const dimensions = displayHypothesisDimensions(match);
  return (
    <article className="rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--color-surface-secondary)] p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {dimensions.map((dimension) => (
          <div key={dimension.key} className="rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--bg-glass)] p-3" title={dimension.explanation}>
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[color:var(--color-accent-action-muted)]">{dimension.label}</p>
            <p className="mt-1 font-display text-lg font-semibold theme-text">{dimension.labelText}</p>
            {typeof dimension.score === "number" ? <p className="mt-1 text-xs theme-muted">{Math.round(dimension.score)}/100</p> : null}
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button type="button" className="organic-button" onClick={onExperiment}><ShieldAlert size={16} /> Test this career</button>
        <button type="button" className="organic-button-secondary" onClick={onAlternative}><RefreshCw size={16} /> Request alternative</button>
        <button type="button" className="organic-button-secondary" onClick={onRoadmap}>Add exploratory action to My Roadmap</button>
...
export function CareerCompatibilityPage() {
  ...
  async function testCareer(match: CareerMatch) {
    if (!id) return;
    const created = await createCareerExperiment(id, {
    ...
    navigate(`/workspace/${id}/experiments/${created.id}`);
  }
  ...
  if (!id) return <ProfileRequiredState title="Create your profile before viewing career hypotheses." />;
  ...
  return (
    <div className="organic-page">
      <section className="organic-section">
        ...
        {evidenceSufficientRecommendation?.state === "evidence_sufficient" ? (
          <section className="mt-4 rounded-2xl border border-[color:var(--color-accent-success)] p-4" data-testid="evidence-sufficient-state">
            <h2 className="font-bold theme-text">Current priority evidence is sufficient</h2>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm theme-muted">{evidenceSufficientRecommendation.rationale.map((item) => <li key={item}>{item}</li>)}</ul>
            <p className="mt-3 text-sm font-semibold theme-text">Choose a next step when you are ready</p>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-sm theme-muted">{(evidenceSufficientRecommendation.next_options || []).map((item) => <li key={item}>{item}</li>)}</ul>
            <p className="mt-3 text-xs theme-muted">No roadmap action was created. Adding one requires explicit confirmation.</p>
          </section>
        ) : null}
      </section>
      ...
      <section className="glass-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <h2 className="font-display text-3xl font-semibold theme-text">Compatible career families</h2>
            <p className="mt-2 text-sm theme-muted">Each career direction is a hypothesis with separate Natural Fit, Capability Fit, Evidence Strength, and Transition Feasibility dimensions.</p>
          </div>
              ...
              <div className="mt-4 grid gap-4">
                {(grouped[category] || []).map((match) => (
                  <MatchCard
                    ...
                    onExperiment={() => void testCareer(match)}
                  />
                ))}
