...
export function MarketApplicationPage() {
  ...
  const selectedAnalysis = useMemo(() => analyses.find((item) => item.id === analysisId) || analyses[0] || null, [analyses, analysisId]);
  const selectedApplication = useMemo(() => applications.find((item) => item.id === applicationId) || applications[0] || null, [applications, applicationId]);
  const analysisDocuments = useMemo(
    () => documents.filter((document) => !selectedAnalysis || document.job_analysis_id === selectedAnalysis.id || !document.job_analysis_id),
  ...
  async function refresh(nextQuery = query, nextMunicipality = municipality) {
    ...
    const [providerResult, radarResult, analysesResult, docsResult, appsResult, researchResult] = await Promise.allSettled([
      ...
      getResearchEvaluation(profileId),
    ]);
    ...
    const failed = [providerResult, radarResult, analysesResult, docsResult, appsResult, researchResult].filter((item) => item.status === "rejected").length;
