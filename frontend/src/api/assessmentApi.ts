...
export async function getCurrentAssessmentSession(profileId: string) {
  const { data } = await apiClient.get<{ session: AssessmentSession | null; definition: AssessmentDefinition; disclaimer: string }>(
...
export async function createAssessmentSession(profileId: string, mode: AssessmentMode, consentAccepted: boolean) {
  const { data } = await apiClient.post<{ session: AssessmentSession; definition: AssessmentDefinition; disclaimer: string }>(
