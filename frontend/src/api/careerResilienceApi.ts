...
export async function createCareerExperiment(
  ...
  const { data } = await apiClient.post<CareerExperimentSession | CareerEvidenceSufficientState>(`/v1/profiles/${profileId}/career-experiments`, payload);
  return data;
}
...
export async function confirmCareerExperimentRoadmap(sessionId: string, confirmed = true) {
  const { data } = await apiClient.post<CareerExperimentSession>(
    `/v1/career-experiment-sessions/${sessionId}/roadmap`,
    { confirmed },
  );
  return data;
}
