        ...
        {isCoachOpen && (
          ...
          >
            ...
            <div className="floating-voice-chat__messages flex-1 space-y-3 overflow-y-auto p-4">
              ...
              {coach.isSending ? <p aria-live="polite" className="rounded-xl bg-slate-100 p-3 text-xs theme-muted">OrganicAI Coach is preparing your response…</p> : null}

              {coach.error ? (
                <div role="alert" className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800">
                  <p>{coach.error}</p>
                  {coach.canRetryLastMessage ? <button type="button" onClick={() => void coach.retryLastMessage()} className="mt-2 underline">Retry message</button> : null}
                </div>
              ) : null}
              ...
              {live.isConnected ? (
                <div className="grid gap-2">
                  ...
                  <div className="floating-voice-chat__composer flex gap-2">
                    ...
                    />
                    <button type="button" aria-label="Send" onClick={sendInput} disabled={coach.isSending} className="organic-button"><Send size={18} /></button>
                  </div>
                </div>
                ...
                <div className="floating-voice-chat__composer flex gap-2">
                  <input
                    ...
                    disabled={coach.isSending}
                  />
                  <button type="button" aria-label="Start live conversation" onClick={requestLive} disabled={coach.isSending} className="organic-button-secondary"><Mic size={18} /></button>
                  <button type="button" aria-label="Send" onClick={sendInput} disabled={coach.isSending} className="organic-button"><Send size={18} /></button>
                </div>
              )}
