...
const earlierApplication = {
  ...app,
  id: "app-earlier",
  job_analysis_id: "analysis-earlier",
  title: "Earlier unrelated role",
  organisation: "Earlier Studio",
  next_action: "Do not use this record for the selected interview.",
};
...
async function mockInterviewJourney(page: Page) {
  ...
  await page.route((url) => url.pathname.startsWith("/api/"), async (route) => {
    ...
    if (pathname === `/api/v1/profiles/${profileId}/applications`) return route.fulfill({ json: [earlierApplication, app] });
    ...
    if (pathname === `/api/v1/profiles/${profileId}/interviews` && method === "POST") {
      expect(route.request().postDataJSON().application_id).toBe(app.id);
...
test("interview journey covers preparation, text mock, reflection, stories, and offer review", async ({ page }) => {
  ...
  await page.getByLabel("Application for this interview").selectOption(app.id);
