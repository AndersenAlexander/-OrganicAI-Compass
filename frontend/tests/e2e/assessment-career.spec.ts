...
async function mockAssessment(page: Page, options: { evidenceSufficient?: boolean } = {}) {
  const calibratedMatches = options.evidenceSufficient
    ? matches.map((match) => match.id === "match-ai-product" ? {
        ...match,
        hypothesis_dimensions: {
          ...match.hypothesis_dimensions,
          scores: { ...match.hypothesis_dimensions.scores, capability_fit: 61, evidence_strength: 78 },
          labels: { ...match.hypothesis_dimensions.labels, capability_fit: "Moderate fit", evidence_strength: "Strong fit" },
        },
        dimension_scores: { ...match.dimension_scores, capability_fit: 61, evidence_strength: 78 },
        dimension_labels: { ...match.dimension_labels, capability_fit: "Moderate fit", evidence_strength: "Strong fit" },
      } : match)
    : matches;
  ...
  await page.route((url) => url.pathname.startsWith("/api/"), async (route) => {
    ...
    if (url.pathname === `/api/v1/assessment-sessions/session-1/complete`) return route.fulfill({ json: { status: "completed", session: results.session, results, career_matches: calibratedMatches } });
    if (url.pathname === `/api/v1/profiles/${profileId}/assessment-results`) return route.fulfill({ json: results });
    if (url.pathname === `/api/v1/profiles/${profileId}/career-matches`) return route.fulfill({ json: calibratedMatches });
    if (url.pathname === `/api/v1/profiles/${profileId}/career-experiments` && route.request().method() === "POST") return route.fulfill({ json: { status: "evidence_sufficient", profile_id: profileId, career_match_id: "match-ai-product", hypothesis_id: "hypothesis-ai-product", user_confirmed: true, expected_evidence_gain: "None", recommendation: { version: "adaptive-career-experiment-ranking-v1", state: "evidence_sufficient", rank: null, score: null, score_breakdown: {}, targeted_gap_skill_ids: [], unresolved_gap_skill_ids: [], already_practically_verified_skill_ids: ["ideation"], rationale: ["Current priority evidence gaps are sufficiently covered.", "No additional skill experiment is recommended solely to keep this loop running."], next_options: ["Collect external evidence or feedback for this direction.", "Add a roadmap action only after explicit confirmation."], ranked_template_ids: [] } } });
    if (url.pathname === `/api/v1/profiles/${profileId}/career-comparisons`) return route.fulfill({ json: { id: "comparison-1", profile_id: profileId, match_ids: ["match-current", "match-ai-product"], criteria_weights: {}, decision_priorities: {}, matrix: { items: calibratedMatches.map((match) => ({ match_id: match.id, title: match.title, alignment_label: match.alignment_label, strengths: match.supporting_factors, challenges: match.conflicting_factors, uncertainties: [], next_experiment: match.next_step, evidence_required: match.missing_skills.length ? match.missing_skills : ["Interview a professional"], criteria: {} })) }, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" } });
    if (url.pathname.includes("/save") || url.pathname.includes("/reject") || url.pathname.includes("/request-alternative")) return route.fulfill({ json: calibratedMatches[1] });
    if (url.pathname.includes("/create-roadmap-draft")) return route.fulfill({ json: { roadmap_id: "roadmap-1", career_match: calibratedMatches[1], actions: [{ id: "action-1", title: "Validate the direction", horizon: "seven_days", status: "not_started" }] } });
...
test("Career Compatibility shows recalibrated evidence dimensions and an evidence-sufficient test outcome", async ({ page }) => {
  await mockAssessment(page, { evidenceSufficient: true });
  await page.goto(`/workspace/${profileId}/career-compatibility`);

  const aiProductCard = page.locator("article").filter({ hasText: "AI Product Designer" }).filter({ hasText: "Natural Fit" });
  await expect(aiProductCard.getByText("86/100")).toBeVisible();
  await expect(aiProductCard.getByText("61/100")).toBeVisible();
  await expect(aiProductCard.getByText("78/100")).toBeVisible();
  await expect(aiProductCard.getByText("64/100")).toBeVisible();
  await expect(aiProductCard.getByText("82/100")).toBeVisible();

  await aiProductCard.getByRole("button", { name: "Test this career" }).click();
