...
const results = {
  ...
  scoring_version: "career-scoring-v1",
  session: { id: "session-1", profile_id: profileId, mode: "quick", status: "completed", consent_accepted: true, assessment_version: "career-assessment-v1", scoring_version: "career-scoring-v1", source_type: "user", demo_marker: false, metadata: {}, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" },
  scores: [],
  grouped_scores: {
...
async function mockAssessment(page: Page) {
  await page.addInitScript((id) => {
    ...
    localStorage.setItem("organicai.auth.token", "token");
  }, profileId);
  await page.route((url) => url.pathname.startsWith("/api/"), async (route) => {
    ...
    if (url.pathname === `/api/v1/profiles/${profileId}/assessment-sessions/current`) return route.fulfill({ json: { session: null, definition, disclaimer: definition.disclaimer } });
    if (url.pathname === `/api/v1/profiles/${profileId}/assessment-sessions`) return route.fulfill({ json: { session: { ...results.session, status: "in_progress", completed_at: null, responses: [] }, definition, disclaimer: definition.disclaimer } });
    ...
    if (url.pathname.endsWith("/auth/me")) return route.fulfill({ json: { id: "user", email: "user@example.test", name: "Alex User", is_demo: false } });
