...
async function expectOkResponse(response: APIResponse, label: string) {
  expect(response.ok(), `${label} failed with status ${response.status()}`).toBeTruthy();
}

test("complete adaptive roadmap flow with partial recalibration", async ({ page, request }, testInfo) => {
  testInfo.setTimeout(180_000);
  const stamp = `e2e-${Date.now()}`;
  const credentials = { email: `${stamp}@example.test`, password: "E2e-roadmap-2026!" };
  const api = process.env.PLAYWRIGHT_API_BASE_URL ?? "http://127.0.0.1:8021/api";
  ...
  const register = await apiRequest.post(`${api}/auth/register`, { data: { name:"Roadmap E2E", ...credentials } });
  await expectOkResponse(register, "API account registration");
  const auth = await register.json();
  const headers = { Authorization:`Bearer ${auth.access_token}` };
  const browserLogin = await page.context().request.post(`${api}/auth/login`, { data: credentials });
  await expectOkResponse(browserLogin, "Browser account login");
  const diagnostic = await apiRequest.post(`${api}/diagnostics`, { headers, data: {
  ...
  await expectOkResponse(diagnostic, "Diagnostic creation");
  ...
  const recommendationList = await apiRequest.get(`${api}/recommendations/profile/${profileId}`, { headers });
  await expectOkResponse(recommendationList, "Recommendation list");
  let recommendations = await recommendationList.json();
  if (!recommendations.length) {
    const generated = await apiRequest.post(`${api}/recommendations/generate`, { headers, data:{ profile_id:profileId } });
    await expectOkResponse(generated, "Recommendation generation");
  ...
  const actionsAfterAdd = await apiRequest.get(`${api}/roadmaps/${roadmapId}/actions`, { headers });
  ...
  for (let index=0; index<4; index++) {
    const seeded = await apiRequest.post(`${api}/roadmaps/${roadmapId}/actions`, { headers, data:{ title:`${stamp} seeded action ${index+1}`, horizon:"seven_days", first_step:"Run a small deterministic step.", success_criteria:"Record the result.", priority:index+10, source_type:"user_created" } });
  ...
  const linkedRecommendation = await apiRequest.get(`${api}/recommendations/${recommendationId}`, { headers });
  ...
  const beforeProposal = await (await apiRequest.get(`${api}/roadmaps/${roadmapId}`, { headers })).json();
  const versionBefore = beforeProposal.version as number;
  const actionsBefore = await (await apiRequest.get(`${api}/roadmaps/${roadmapId}/actions`, { headers })).json();
  ...
  const unchanged = await (await apiRequest.get(`${api}/roadmaps/${roadmapId}/actions`, { headers })).json();
  ...
  actions = await (await apiRequest.get(`${api}/roadmaps/${roadmapId}/actions`, { headers })).json();
  ...
  const versionsResponse = await apiRequest.get(`${api}/roadmaps/${roadmapId}/versions`, { headers }); expect(versionsResponse.status()).toBe(200);
  ...
  const checkinsApi = await apiRequest.get(`${api}/roadmaps/${roadmapId}/check-ins`, { headers }); expect(checkinsApi.status()).toBe(200);
