...
from app.models.innovation_extension import CareerDecisionJournalEntry, CareerDecisionJournalVersion
...
INTERVIEW_OUTCOMES = {"NEXT_STAGE", "OFFER", "REJECTED", "WITHDRAWN", "NO_DECISION", "UNKNOWN"}

QUESTION_STATUSES = {"active", "kept", "edited", "dismissed", "archived", "custom"}
...
STAR_SUPPORT_STATES = {"SUPPORTED", "PARTIALLY_SUPPORTED", "SELF_REPORT_ONLY", "UNSUPPORTED", "NEEDS_REVIEW"}

MOCK_MODES = {"guided_practice", "full_simulation", "single_question"}

PANEL_PERSONAS = {
    "recruiter": {"label": "Recruiter", "purpose": "Explore logistics, motivation, and application context."},
    "hiring_manager": {"label": "Hiring Manager", "purpose": "Explore role-specific judgement, collaboration, and contribution."},
    "technical_lead": {"label": "Technical Lead", "purpose": "Explore technical decisions, testing, trade-offs, and limitations."},
    "product_lead": {"label": "Product Lead", "purpose": "Explore users, prioritisation, metrics, and product trade-offs."},
    "portfolio_reviewer": {"label": "Portfolio Reviewer", "purpose": "Explore project ownership, process, evidence, and learning."},
}
...
def _normalise_lifecycle_status(status: str | None) -> str:
    value = re.sub(r"[^a-z0-9]+", "_", (status or "PLANNED").strip().lower()).strip("_").upper()
    if value not in INTERVIEW_LIFECYCLE_STATUSES:
        raise ValueError("Unsupported interview lifecycle status.")
    return value


def _normalise_outcome(outcome: str | None) -> str:
    value = re.sub(r"[^a-z0-9]+", "_", (outcome or "UNKNOWN").strip().lower()).strip("_").upper()
    aliases = {"NEXT_STAGE_RECEIVED": "NEXT_STAGE", "NEXT_INTERVIEW": "NEXT_STAGE", "NO_DECISION_YET": "NO_DECISION"}
    value = aliases.get(value, value)
    if value not in INTERVIEW_OUTCOMES:
        raise ValueError("Unsupported interview outcome.")
    return value


def _normalise_question_type(value: str | None, category: str | None = None) -> str:
    raw = value or category or "experience"
    normalized = re.sub(r"[^a-z0-9]+", "_", str(raw).strip().lower()).strip("_")
    aliases = {
        "behaviour": "behavioural",
        "behavioral": "behavioural",
        "evidence_verification": "evidence_clarification",
        "candidate_question": "candidate_questions",
        "role_knowledge": "role_understanding",
        "career_transition_explanation": "motivation",
    }
    normalized = aliases.get(normalized, normalized)
    return normalized if normalized in QUESTION_TYPES else "experience"


def _requirement_set_version(db: Session, analysis: JobAnalysis | None) -> str:
    if not analysis:
        return "unlinked"
    return str(getattr(analysis, "extraction_version", None) or "job-analysis-v1")


def _interview_source_snapshot(interview: Interview) -> dict[str, Any]:
    return {
        "interview_id": interview.id,
        "profile_id": interview.profile_id,
        "application_id": interview.application_id,
        "job_analysis_id": interview.job_analysis_id,
        "requirement_set_version": interview.requirement_set_version,
...
def _confirmed_requirements(db: Session, interview: Interview) -> list[JobRequirement]:
    """Return only requirements that are final-confirmed in the linked analysis.

    Interview preparation may show a safeguard when confirmation is missing, but
    it must not present extracted requirements as an authoritative job source.
    """
    if not interview.job_analysis_id:
        return []
    analysis = db.get(JobAnalysis, interview.job_analysis_id)
    if not analysis or not analysis.user_confirmed:
        return []
    return [
        requirement
        for requirement in _requirements(db, interview.job_analysis_id)
        if requirement.user_confirmation_state == "confirmed"
    ]
...
def create_interview(db: Session, profile: Profile, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    lifecycle_status = _normalise_lifecycle_status(payload.get("status") or payload.get("lifecycle_status"))
    outcome = _normalise_outcome(payload.get("outcome") or payload.get("interview_result"))
    application = None
    analysis = None
    ...
    if application and payload.get("job_analysis_id") and application.job_analysis_id != payload.get("job_analysis_id"):
        raise ValueError("Interview job analysis must match the linked application.")
    ...
    # The tracker action is safe to retry: the same application, stage, and
    # scheduled appointment should continue the existing planned interview,
    # not create a duplicate journey record or a second application event.
    if application and scheduled_at:
        existing = db.scalar(
            select(Interview)
            .where(
                Interview.profile_id == profile.id,
                Interview.application_id == application.id,
                Interview.stage_type == stage,
                Interview.scheduled_at == scheduled_at,
                Interview.status.in_({"PLANNED", "IN_PROGRESS"}),
            )
            .order_by(Interview.updated_at.desc(), Interview.created_at.desc())
        )
        if existing:
            return interview_public(db, existing)
    ...
    interview = Interview(
        ...
        role=_clean_text(payload.get("role") or payload.get("title") or (application.title if application else analysis.title if analysis else ""), 255),
        stage_type=stage,
        stage_order=int(payload.get("stage_order") or _stage_count(db, profile.id, application.id if application else None) + 1),
        status=lifecycle_status,
        requirement_set_version=payload.get("requirement_set_version") or _requirement_set_version(db, analysis),
        ...
        version_metadata_json={
            "created_from": payload.get("source") or ("application" if application else "manual"),
            "requirement_set_version": payload.get("requirement_set_version") or _requirement_set_version(db, analysis),
            "user_confirmed": bool(payload.get("user_confirmed", False)),
        },
        outcome_source=_clean_text(payload.get("outcome_source") or "", 120),
        outcome_reason=_clean_text(payload.get("outcome_reason") or "", 4000),
    )
    db.add(interview)
    db.flush()
    if application:
...
def update_interview(db: Session, interview: Interview, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("status") is not None or payload.get("lifecycle_status") is not None:
        interview.status = _normalise_lifecycle_status(payload.get("status") or payload.get("lifecycle_status"))
    for key, limit in [
        ("organisation", 255),
        ("role", 255),
        ("timezone", 80),
    ...
    if "interview_result" in payload or "outcome" in payload:
        interview.interview_result = _normalise_outcome(payload.get("outcome") or payload.get("interview_result"))
    if "outcome_source" in payload:
        interview.outcome_source = _clean_text(str(payload.get("outcome_source") or ""), 120)
    if "outcome_reason" in payload:
        interview.outcome_reason = _clean_text(str(payload.get("outcome_reason") or ""), 4000)
    if "requirement_set_version" in payload and payload.get("requirement_set_version") is not None:
        interview.requirement_set_version = _clean_text(str(payload["requirement_set_version"]), 120)
    if "version_metadata" in payload and isinstance(payload.get("version_metadata"), dict):
        interview.version_metadata_json = payload["version_metadata"]
    interview.version_number += 1
...
def interview_public(db: Session, row: Interview, include_details: bool = False) -> dict[str, Any]:
    ...
    payload = {
        ...
        "requirement_set_version": row.requirement_set_version,
        ...
        "version_metadata": row.version_metadata_json or {},
...
def interview_dashboard(db: Session, profile: Profile) -> dict[str, Any]:
    ...
    upcoming = [item for item in interviews if item["scheduled_at"] and item.get("status", "PLANNED") not in {"COMPLETED", "CANCELLED", "NO_SHOW", "ARCHIVED"}]
    pending_reflections = [item for item in interviews if item.get("status") == "COMPLETED" and not item["has_reflection"]]
    active_preparation = [
        item
        for item in interviews
        if item["preparation_status"] in {"Not started", "In progress", "Needs evidence", "Ready for practice"}
        and item.get("status", "PLANNED") not in {"COMPLETED", "CANCELLED", "NO_SHOW", "ARCHIVED"}
    ]
    completed_with_reflection = [item for item in interviews if item.get("status") == "COMPLETED" and item.get("has_reflection")]
    pending_outcomes = [
        item
        for item in completed_with_reflection
        if not str(item.get("interview_result") or "").strip()
        or str(item.get("interview_result") or "").strip().upper() in {"UNKNOWN", "NO_DECISION"}
    ]
    offer_review_interview_ids = set(
        db.scalars(select(OfferReview.interview_id).where(OfferReview.profile_id == profile.id, OfferReview.interview_id.is_not(None))).all()
    )
    offers_needing_review = [
        item
        for item in completed_with_reflection
        if str(item.get("interview_result") or "").upper() == "OFFER" and item["id"] not in offer_review_interview_ids
    ]
    pending_recalibrations = db.scalar(
        select(func.count())
        .select_from(ApplicationRecalibrationRun)
        .where(
            ApplicationRecalibrationRun.profile_id == profile.id,
            ApplicationRecalibrationRun.user_decision.in_([None, "", "PENDING"]),
        )
    ) or 0
    gaps = Counter()
    for interview in interviews:
        if not interview["job_analysis_id"]:
            ...
            continue
        interview_row = db.get(Interview, interview["id"])
        confirmed_requirements = _confirmed_requirements(db, interview_row) if interview_row else []
        if not confirmed_requirements:
            gaps["job analysis awaiting final confirmation"] += 1
            continue
    ...
    if not applications:
        next_action = "Create or save an application before adding an interview."
    elif not interviews:
        next_action = "Create an Interview Journey record or add an interview."
    elif upcoming or active_preparation:
        next_action = "Prepare for your next interview."
    elif pending_reflections:
        next_action = "Record a post-interview reflection."
    elif pending_outcomes:
        next_action = "Record the interview outcome."
    elif offers_needing_review:
        next_action = "Review the recorded offer facts and remaining unknowns."
    elif pending_recalibrations:
        next_action = "Review the recalibration proposal."
    else:
        next_action = "Create or select an interview to begin preparation."
    return {
        "profile_id": profile.id,
...
def _confirmed_requirements_payload(db: Session, interview: Interview) -> list[dict[str, Any]]:
    requirements = []
    for requirement in _confirmed_requirements(db, interview):
        requirements.append(
            {
                "id": requirement.id,
...
def _application_document_claims(db: Session, interview: Interview) -> list[dict[str, Any]]:
    """Expose application claims as review targets without treating them as facts."""
    document_ids = [item for item in [interview.cv_document_id, interview.cover_letter_document_id] if item]
    if not document_ids:
        return []
    documents = {
        row.id: row
        for row in db.scalars(
            select(ApplicationDocument).where(
                ApplicationDocument.id.in_(document_ids),
                ApplicationDocument.profile_id == interview.profile_id,
            )
        ).all()
    }
    claims = db.scalars(
        select(DocumentClaim)
        .where(DocumentClaim.document_id.in_(list(documents)), DocumentClaim.profile_id == interview.profile_id)
        .order_by(DocumentClaim.updated_at.desc())
    ).all() if documents else []
    return [
        {
            "document": documents[claim.document_id].document_type.replace("_", " ").title(),
            "claim": claim.claim_text,
            "support_state": claim.support_state,
            "status": claim.status,
            "requires_careful_support": claim.blocked_for_export or claim.status not in {"Supported", "User-confirmed"},
        }
...
def generate_preparation_brief(db: Session, interview: Interview, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    document_claims = _application_document_claims(db, interview)
    claims_needing_support = [item for item in document_claims if item["requires_careful_support"]]
    transferable = []
    ...
    sections = {
        ...
        "application_claims_to_support": {
            "confirmed_facts": [item for item in document_claims if not item["requires_careful_support"]][:8],
            "missing_information": claims_needing_support[:8] or (["No current CV or Cover Letter claims are linked to this interview."] if not document_claims else []),
            "source": "Current CV / Cover Letter Evidence Lock",
        },
    ...
    brief.requirement_set_version = interview.requirement_set_version
    brief.version_number = int(brief.version_number or 0) + 1
    brief.source_snapshot_json = {
        **_interview_source_snapshot(interview),
        "sources": ["confirmed job requirements", "Evidence Passport", "application documents", "employer context"],
        "confirmed_requirement_count": len(requirements),
        "supported_requirement_count": len(supported_requirements),
        "application_claim_count": len(document_claims),
    }
...
def preparation_public(row: InterviewPreparationBrief) -> dict[str, Any]:
    return {
        ...
        "requirement_set_version": row.requirement_set_version,
        "version_number": row.version_number,
        "source_snapshot": row.source_snapshot_json or {},
        "demo_marker": row.demo_marker,
        "created_at": row.created_at.isoformat(),
        "updated_at": row.updated_at.isoformat(),
    }
...
def generate_interview_questions(db: Session, interview: Interview, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    requirements = _confirmed_requirements(db, interview)
    known_question_texts = {
        re.sub(r"\s+", " ", row.question_text.strip().lower())
        for row in existing
    }

    def add_question_if_new(row: InterviewQuestion) -> None:
        key = re.sub(r"\s+", " ", row.question_text.strip().lower())
        if key in known_question_texts:
            return
        db.add(row)
        created.append(row)
        known_question_texts.add(key)
    ...
    for template in _base_questions(interview.stage_type):
        row = InterviewQuestion(
            ...
            question_type=_normalise_question_type(template["category"]),
        )
        add_question_if_new(row)
    for requirement in requirements[:8]:
        ...
        row = InterviewQuestion(
            ...
            related_evidence_gap_json={"requirement": requirement.requirement_text, "has_linked_evidence": bool(evidence), "reason": "No linked Evidence Passport match."} if not evidence else {},
        )
        add_question_if_new(row)
    if not requirements:
        row = InterviewQuestion(
            ...
            origin="deterministic",
            question_type="role_understanding",
            related_evidence_gap_json={"reason": "No confirmed job analysis requirements are available."},
        )
        add_question_if_new(row)
    db.flush()
    ...
    interview.updated_at = _now()
    db.commit()
    current_questions = db.scalars(select(InterviewQuestion).where(InterviewQuestion.interview_id == interview.id).order_by(InterviewQuestion.created_at)).all()
    return {"interview_id": interview.id, "questions": [question_public(item) for item in current_questions], "generated": bool(created)}
...
def question_public(row: InterviewQuestion) -> dict[str, Any]:
    return {
        ...
        "label": "Plausible practice question",
        ...
        "custom": row.custom,
        "related_evidence_gap": row.related_evidence_gap_json or {},
        "rationale": {
            "heading": "WHY THIS QUESTION APPEARS",
            "text": row.why_it_may_be_asked,
            "linked_requirement": row.related_job_requirement,
            "linked_evidence": row.related_evidence_json or [],
            "linked_evidence_gap": row.related_evidence_gap_json or {},
        },
        "version": row.version,
        "created_at": row.created_at.isoformat(),
        "updated_at": row.updated_at.isoformat(),
...
def save_question(db: Session, question: InterviewQuestion, saved: bool = True) -> dict[str, Any]:
    ...
    question.question_status = "kept" if saved else "active"
    question.updated_at = _now()
    db.commit()
    return question_public(question)


def update_interview_question(db: Session, question: InterviewQuestion, payload: dict[str, Any]) -> dict[str, Any]:
    if "question_text" in payload:
        text = _clean_text(str(payload.get("question_text") or ""), 2000)
        if not text:
            raise ValueError("Question text cannot be empty.")
        question.question_text = text
        question.user_edited = True
    if "question_type" in payload or "category" in payload:
        question.question_type = _normalise_question_type(payload.get("question_type"), payload.get("category") or question.category)
    if "category" in payload and payload.get("category"):
        question.category = _clean_text(str(payload["category"]), 80)
    if "status" in payload or "question_status" in payload:
        status = str(payload.get("status") or payload.get("question_status") or "active").strip().lower()
        if status not in QUESTION_STATUSES:
            raise ValueError("Unsupported question status.")
        question.question_status = status
    if "why_it_may_be_asked" in payload:
        question.why_it_may_be_asked = _clean_text(str(payload.get("why_it_may_be_asked") or ""), 4000)
        question.user_edited = True
    if "saved" in payload:
        question.saved_by_user = bool(payload["saved"])
        question.question_status = "kept" if question.saved_by_user else question.question_status
    question.version += 1
    question.updated_at = _now()
    db.commit()
    return question_public(question)
...
def create_custom_question(db: Session, interview: Interview, payload: dict[str, Any]) -> dict[str, Any]:
    text = _clean_text(payload.get("question_text") or payload.get("text") or "", 2000)
    if not text:
        raise ValueError("Custom question text is required.")
    category = _clean_text(payload.get("category") or "experience", 80)
    requirement_id = payload.get("related_job_requirement_id")
    requirement = db.get(JobRequirement, requirement_id) if requirement_id else None
    if requirement_id and (not requirement or requirement.profile_id != interview.profile_id or requirement.analysis_id != interview.job_analysis_id):
        raise PermissionError("Linked requirement does not belong to this interview context.")
    row = InterviewQuestion(
        interview_id=interview.id,
        profile_id=interview.profile_id,
        application_id=interview.application_id,
        ...
        stage=interview.stage_type,
        question_text=text,
        why_it_may_be_asked="Added by the user; no employer prediction is implied.",
        answer_objective=_clean_text(payload.get("answer_objective") or "Answer with confirmed facts and clearly labelled limitations.", 2000),
        risk_level="medium",
        difficulty=_clean_text(payload.get("difficulty") or "moderate", 40),
        source_type="user_created",
        origin="user",
        question_type=_normalise_question_type(payload.get("question_type"), category),
        question_status="custom",
        saved_by_user=True,
        user_edited=True,
        custom=True,
        related_job_requirement_id=requirement.id if requirement else None,
        related_job_requirement=_clean_text(payload.get("related_job_requirement") or (requirement.requirement_text if requirement else ""), 1200),
        related_evidence_gap_json=payload.get("related_evidence_gap") if isinstance(payload.get("related_evidence_gap"), dict) else {},
    )
    db.add(row)
    interview.preparation_status = "In progress"
    interview.updated_at = _now()
    db.commit()
    return question_public(row)
...
def _story_snapshot(story: StarStory) -> dict[str, Any]:
    return {
        ...
        "support_state": story.support_state,
        "linked_capability": story.linked_capability,
        "canonical_story_id": story.canonical_story_id,
        "adaptation_context": story.adaptation_context_json or {},
...
def create_star_story(db: Session, profile: Profile, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    canonical_story_id = payload.get("canonical_story_id")
    if canonical_story_id:
        canonical = require_star_story(db, str(canonical_story_id))
        if canonical.profile_id != profile.id:
            raise PermissionError("Canonical STAR story does not belong to this profile.")
    story = StarStory(
        profile_id=profile.id,
        user_id=user_id or profile.user_id,
        ...
        linked_capability=_clean_text(payload.get("linked_capability") or ((payload.get("skills_demonstrated") or [""])[0]), 160),
        canonical_story_id=canonical_story_id,
        adaptation_context_json=payload.get("adaptation_context") if isinstance(payload.get("adaptation_context"), dict) else {},
    )
    db.add(story)
    db.flush()
    ...
    story.support_state = quality["support_state"]
...
def update_star_story(db: Session, story: StarStory, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if "linked_capability" in payload:
        story.linked_capability = _clean_text(str(payload.get("linked_capability") or ""), 160)
    if "adaptation_context" in payload and isinstance(payload.get("adaptation_context"), dict):
        story.adaptation_context_json = payload["adaptation_context"]
...
def adapt_star_story(db: Session, story: StarStory, payload: dict[str, Any]) -> dict[str, Any]:
    """Create a job-specific copy while preserving the canonical story."""
    adaptation = {
        "title": payload.get("title") or f"{story.title} — job-specific adaptation",
        "situation": payload.get("situation") or story.situation,
        "task": payload.get("task") or story.task,
        "action": payload.get("action") or story.action,
        "result": payload.get("result") or story.result,
        "reflection": payload.get("reflection") or story.reflection,
        "skills_demonstrated": payload.get("skills_demonstrated") or story.skills_demonstrated_json,
        "related_job_requirements": payload.get("related_job_requirements") or story.related_job_requirements_json,
        "evidence_links": payload.get("evidence_links") or story.evidence_links_json,
        "tags": payload.get("tags") or story.tags_json,
        "confidentiality_status": payload.get("confidentiality_status") or story.confidentiality_status,
        "linked_capability": payload.get("linked_capability") or story.linked_capability,
        "canonical_story_id": story.id,
        "adaptation_context": {
            "adapted_for": payload.get("adapted_for") or "interview question",
            "interview_id": payload.get("interview_id"),
            "question_id": payload.get("question_id"),
            "source_version": story.version,
        },
        "source": "job_specific_adaptation",
        "user_confirmed": bool(payload.get("user_confirmed", False)),
    }
    return create_star_story(db, db.get(Profile, story.profile_id), adaptation, story.user_id)
...
def evaluate_star_story_row(story: StarStory) -> dict[str, Any]:
    checks = []
    evidence_links = [item for item in (story.evidence_links_json or []) if isinstance(item, dict)]
    verified_evidence_links = [item for item in evidence_links if item.get("evidence_id") or item.get("verification_status") in {"supported", "verified"}]
    scores = {
        ...
        "result evidence": 4 if verified_evidence_links and story.result else 2 if evidence_links and story.result else 2 if story.result else 0,
    ...
    unsupported = metric_claim and not verified_evidence_links and not story.user_confirmed
    if unsupported:
        ...
        claim_statuses.append(
            {
                ...
                "status": "Supported" if verified_evidence_links else "Partially supported" if evidence_links else "User-confirmed" if story.user_confirmed else "Unverified",
    ...
    if verified_evidence_links and unsupported:
        support_state = "PARTIALLY_SUPPORTED"
    elif verified_evidence_links:
        support_state = "SUPPORTED"
    elif evidence_links:
        support_state = "PARTIALLY_SUPPORTED"
    elif unsupported:
        support_state = "UNSUPPORTED"
    elif story.user_confirmed:
        support_state = "SELF_REPORT_ONLY"
    else:
    ...
    return {
        ...
        "support_explanation": {
            ...
            "verified_evidence_link_count": len(verified_evidence_links),
            "warning": "Polished wording is not evidence." if support_state != "SUPPORTED" else "Evidence links are present; verify their scope before use.",
        },
        "deterministic_version": "star-story-quality-v2",
    }
...
def star_story_public(row: StarStory) -> dict[str, Any]:
    return {
        ...
        "support_explanation": (row.quality_json or {}).get("support_explanation", {}),
        "linked_capability": row.linked_capability,
        "canonical_story_id": row.canonical_story_id,
        "adaptation_context": row.adaptation_context_json or {},
...
def create_mock_session(db: Session, interview: Interview, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if delivery == "voice" and not get_settings().interview_voice_enabled:
        ...
        fallback = ""
    mode = str(payload.get("mode") or "guided_practice").strip().lower()
    mode_aliases = {"guided": "guided_practice", "full": "full_simulation", "single": "single_question", "voice_disabled_fallback": "guided_practice"}
    mode = mode_aliases.get(mode, mode)
    if mode not in MOCK_MODES:
        raise ValueError("Mock interview mode must be guided_practice, full_simulation, or single_question.")
    requested_personas = payload.get("panel_personas") if isinstance(payload.get("panel_personas"), list) else [payload.get("persona") or _persona_for_stage(interview.stage_type)]
    if any(str(item).strip().lower() == "panel" for item in requested_personas):
        requested_personas = list(PANEL_PERSONAS)
    personas = [str(item).strip().lower() for item in requested_personas if str(item).strip().lower() in PANEL_PERSONAS]
    if not personas:
        personas = [_persona_for_stage(interview.stage_type)]
    ...
    session = MockInterviewSession(
        ...
        panel_personas_json=personas,
        persona_feedback_json={},
...
def start_mock_session(db: Session, session: MockInterviewSession) -> dict[str, Any]:
    ...
    if not questions:
        ...
        questions = db.scalars(select(InterviewQuestion).where(InterviewQuestion.interview_id == interview.id).order_by(InterviewQuestion.created_at)).all()
    limit = 1 if session.mode == "single_question" else 8 if session.mode == "full_simulation" else 5
    session.question_sequence_json = [item.id for item in questions[:limit]]
...
def add_mock_turn(db: Session, session: MockInterviewSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    linked_evidence_count = len(
        [
            item
            for item in (question.related_evidence_json if question else [])
            if isinstance(item, dict) and item.get("evidence_id")
        ]
    )
    rubric = score_answer(
        question.question_text if question else turn.question_text,
        answer_text,
        payload.get("response_duration_seconds"),
    ...
    turn.transcript_source = "voice" if session.delivery_mode == "voice" else "text"
    turn.transcript_storage_status = "retained" if session.transcript_retained else "not_stored"
    turn.user_edited_transcript = bool(payload.get("user_edited_transcript", False))
    if question and question.related_job_requirement:
        turn.feedback_json["evidence_links"] = {
            "question_linked_requirement": question.related_job_requirement,
            "evidence_available": question.related_evidence_json or [],
            "response_referenced_evidence": rubric["feedback"].get("evidence_use_observation", ""),
        }
    db.add(turn)
    session.updated_at = _now()
    db.commit()
...
def score_answer(
    ...
    unsupported_status, _, _, blocked = _claim_status_from_text(answer_text, linked_count=linked_evidence_count)
    unsupported_factual_claim = bool(
        re.search(
            r"\b(?:i|we)\s+(?:managed|led|supervised|grew|increased|delivered|generated|reduced|saved)\s+(?:a\s+)?\d+\b",
            answer_text,
            re.I,
        )
    ) and linked_evidence_count == 0
    ...
    rubric = [
        {"criterion_code": "RELEVANCE", "criterion": "answered the actual question", "score": answered},
        {"criterion_code": "EVIDENCE_USE", "criterion": "used relevant evidence", "score": evidence},
        {"criterion_code": "SPECIFICITY", "criterion": "explained personal contribution", "score": contribution},
        {"criterion_code": "STRUCTURE", "criterion": "provided sufficient context", "score": context},
        {"criterion_code": "COMPLETENESS", "criterion": "described actions", "score": actions},
        {"criterion_code": "RESULT", "criterion": "described result", "score": result},
        {"criterion_code": "UNSUPPORTED_CLAIMS", "criterion": "avoided unsupported claims", "score": unsupported_score},
        {"criterion_code": "LENGTH", "criterion": "stayed within suggested time", "score": timing_score},
        {"criterion_code": "CLARITY", "criterion": "used understandable language", "score": language},
        {"criterion_code": "REFLECTION", "criterion": "included reflection where appropriate", "score": reflection},
    ]
    ...
    return {
        ...
        "feedback": {
            ...
            "unsupported_or_unclear_claims": (
                ["Unsupported factual claim — verify or revise before using this in a real interview."]
                if unsupported_factual_claim
                else [answer_text[:300]] if blocked or unsupported_status == "Unverified" else []
            ),
            "missing_evidence": [] if evidence >= 3 else ["Add one evidence-linked project, experiment, or confirmed experience."],
            "evidence_use_observation": "The answer references an evidence-like example." if evidence >= 3 else "No concrete evidence-linked example was detected in the response.",
...
def complete_mock_session(db: Session, session: MockInterviewSession, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    feedback = {
        ...
        "no_single_opaque_score": True,
        "feedback_basis": "Observable answer characteristics only; no personality, emotion, accent, honesty, or employability assessment.",
        "criteria_version": "interview-rubric-v2",
    }
    persona_feedback = {}
    for persona in session.panel_personas_json or [session.persona]:
        definition = PANEL_PERSONAS.get(persona, {"label": persona, "purpose": "Practice perspective"})
        persona_feedback[persona] = {
            "label": definition["label"],
            "simulated_perspective": True,
            "purpose": definition["purpose"],
            "observations": list(feedback["needs_improvement"][:3]),
            "rubric": aggregate,
            "not_a_prediction": "This is a simulated perspective, not how a real interviewer will evaluate you.",
        }
    session.status = "completed"
    session.completed_at = _now()
    session.transcript_confirmed = bool(payload.get("transcript_confirmed", False))
    session.transcript_retained = session.transcript_confirmed and bool(payload.get("transcript_retained", False))
    for turn in turns:
        turn.transcript_storage_status = "retained" if session.transcript_retained else "not_stored"
    session.rubric_results_json = aggregate
    session.feedback_json = feedback
    session.persona_feedback_json = persona_feedback
    session.updated_at = _now()
    interview = db.get(Interview, session.interview_id)
    if interview:
...
def mock_session_public(db: Session, row: MockInterviewSession) -> dict[str, Any]:
    ...
    return {
        ...
        "persona": row.persona,
        "panel_personas": row.panel_personas_json or [row.persona],
        "persona_feedback": row.persona_feedback_json or {},
        "status": row.status,
        ...
        "text_fallback_available": row.text_fallback_available,
...
def mock_turn_public(row: MockInterviewTurn) -> dict[str, Any]:
    return {
        ...
        "transcript_storage_status": row.transcript_storage_status,
        "user_edited_transcript": row.user_edited_transcript,
...
def create_voice_session(db: Session, profile: Profile, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row = VoiceProviderSession(
        ...
        transcript_storage_status="retained" if payload.get("transcript_retained", False) and settings.interview_voice_transcript_retention_enabled else "not_stored",
...
def complete_voice_session(db: Session, row: VoiceProviderSession, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    confirmed = bool(payload.get("transcript_confirmed", row.consent_confirmed))
    row.transcript_retained = bool(payload.get("transcript_retained", row.transcript_retained) and confirmed and settings.interview_voice_transcript_retention_enabled)
    row.transcript_storage_status = "retained" if row.transcript_retained else "not_stored"
    if not row.transcript_retained:
        row.transcript_text = ""
        row.user_edited_transcript = ""
    row.updated_at = _now()
    db.commit()
    return voice_session_public(row)


def update_voice_transcript(db: Session, row: VoiceProviderSession, payload: dict[str, Any]) -> dict[str, Any]:
    settings = get_settings()
    action = str(payload.get("action") or "save").strip().lower()
    if action in {"reject", "delete", "remove"}:
        row.transcript_text = ""
        row.user_edited_transcript = ""
        row.transcript_retained = False
        row.transcript_storage_status = "deleted"
    else:
        if not payload.get("transcript_confirmed", row.consent_confirmed):
            raise ValueError("Explicit transcript confirmation is required before storage.")
        if not settings.interview_voice_transcript_retention_enabled:
            raise ValueError("Transcript retention is disabled; use text mode or enable retention explicitly.")
        transcript = _clean_text(payload.get("transcript") or "", 20000)
        if not transcript:
            raise ValueError("Transcript text is required.")
        row.transcript_text = transcript
        row.user_edited_transcript = _clean_text(payload.get("user_edited_transcript") or "", 20000)
        row.transcript_retained = True
        row.transcript_storage_status = "retained_user_edited" if row.user_edited_transcript else "retained"
    row.updated_at = _now()
    db.commit()
    return voice_session_public(row)
...
def voice_session_public(row: VoiceProviderSession) -> dict[str, Any]:
    return {
        ...
        "transcript_retained": row.transcript_retained,
        "transcript_storage_status": row.transcript_storage_status,
        "transcript": row.transcript_text if row.transcript_retained else "",
        "user_edited_transcript": row.user_edited_transcript if row.transcript_retained else "",
...
def create_reflection(db: Session, interview: Interview, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row.user_observation = _clean_text(payload.get("user_observation") or "", 5000)
    row.user_interpretation = _clean_text(payload.get("user_interpretation") or "", 5000)
    suggestions = _reflection_suggestions(payload)
    row.ai_interpretation_json = {
        ...
        "possible_preparation_updates": suggestions,
    }
    row.system_suggestion_json = suggestions
    ...
    outcome_supplied = payload.get("outcome_status") is not None or payload.get("outcome") is not None
    row.outcome_status = _normalise_outcome(payload.get("outcome_status") or payload.get("outcome")) if outcome_supplied else (row.outcome_status or "UNKNOWN")
    row.outcome_source = _clean_text(payload.get("outcome_source") or "", 120)
    row.outcome_reason = _clean_text(payload.get("outcome_reason") or "", 5000)
    row.outcome_confirmed = bool(payload.get("outcome_confirmed", False))
    if row.outcome_confirmed and not outcome_supplied:
        raise ValueError("An explicit outcome is required when confirming an interview outcome from reflection.")
    ...
    # Saving a post-interview reflection is an explicit completion action, but
    # never an implicit outcome record.
    interview.status = "COMPLETED"
    if row.outcome_confirmed:
        interview.interview_result = row.outcome_status
        interview.outcome_source = row.outcome_source
        interview.outcome_reason = row.outcome_reason
        interview.status = "COMPLETED"
    interview.updated_at = _now()
    db.add(row)
    app = db.get(JobApplication, interview.application_id) if interview.application_id else None
    if app:
        ...
        if row.outcome_confirmed:
            proposal = db.scalar(select(ApplicationRecalibrationRun).where(ApplicationRecalibrationRun.interview_id == interview.id).order_by(ApplicationRecalibrationRun.created_at.desc()))
            proposal_is_new = proposal is None
            if not proposal:
                proposal = ApplicationRecalibrationRun(
                    application_id=app.id,
                    interview_id=interview.id,
                    profile_id=interview.profile_id,
                    status="suggested",
            ...
            proposal.interview_id = interview.id
            proposal.source_label = "Post-interview reflection and explicit outcome record"
            proposal.user_decision = proposal.user_decision or "PENDING"
            proposal.before_state_json = {
                "career_hypothesis": "unchanged",
                "evidence_passport": "unchanged",
                "human_potential_map": "unchanged",
                "roadmap": "unchanged",
            }
            proposal.after_state_json = {}
            proposal.limitation = "One interview outcome provides limited evidence and does not determine career fit."
            if proposal_is_new:
                db.add(proposal)
    db.commit()
...
def reflection_public(row: InterviewReflection) -> dict[str, Any]:
    return {
        ...
        "outcome_source": row.outcome_source,
        "outcome_reason": row.outcome_reason,
        "outcome_confirmed": row.outcome_confirmed,
        "user_confirmed": row.user_confirmed,
        "created_at": row.created_at.isoformat(),
        "updated_at": row.updated_at.isoformat(),
    }
...
def _recalibration_snapshot(proposal: ApplicationRecalibrationRun) -> dict[str, Any]:
    return {
        "before": proposal.before_state_json or {},
        "after": proposal.after_state_json or {},
...
def recalibration_proposal_public(row: ApplicationRecalibrationRun) -> dict[str, Any]:
    return {
        "id": row.id,
        "interview_id": row.interview_id,
        "application_id": row.application_id,
        ...
        "before": row.before_state_json or {},
        "after": row.after_state_json or {},
...
def list_interview_recalibration_proposals(db: Session, interview: Interview) -> list[dict[str, Any]]:
    rows = db.scalars(select(ApplicationRecalibrationRun).where(ApplicationRecalibrationRun.interview_id == interview.id).order_by(ApplicationRecalibrationRun.created_at.desc())).all()
    return [recalibration_proposal_public(row) for row in rows]


def record_interview_outcome(db: Session, interview: Interview, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    reflection = db.scalar(
        select(InterviewReflection)
        .where(InterviewReflection.interview_id == interview.id)
        .order_by(InterviewReflection.updated_at.desc())
    )
    if not reflection:
        raise ValueError("Record a post-interview reflection before recording an outcome.")
    outcome = _normalise_outcome(payload.get("outcome") or payload.get("outcome_status"))
    source = _clean_text(payload.get("source") or payload.get("outcome_source") or "", 120)
    reason = _clean_text(payload.get("reason") or payload.get("outcome_reason") or "", 5000)
    interview.interview_result = outcome
    interview.outcome_source = source
    interview.outcome_reason = reason
    interview.status = "COMPLETED"
    interview.version_number += 1
    interview.version_metadata_json = {
        **(interview.version_metadata_json or {}),
        "outcome_recorded_at": _now().isoformat(),
        "outcome_user_confirmed": True,
    }
    app = db.get(JobApplication, interview.application_id) if interview.application_id else None
    status_update_confirmed = bool(payload.get("confirm_application_status_update", False))
    ...
    if app:
        event = JobApplicationEvent(
            ...
            from_status=app.status,
            to_status=app.status,
            description="Interview outcome recorded. Application Tracker status changes require explicit confirmation.",
            event_metadata_json={"interview_id": interview.id, "outcome": outcome, "source": source, "status_update_confirmed": status_update_confirmed},
            created_by=user_id,
        )
        db.add(event)
        if status_update_confirmed:
            target = {"OFFER": "Offer", "REJECTED": "Rejected", "WITHDRAWN": "Withdrawn", "NEXT_STAGE": _stage_status(interview.stage_type)}.get(outcome)
            if target in APPLICATION_STATUSES:
                app.status = target
                event.to_status = target
        application_result = application_public(db, app)
    db.commit()
    result = interview_public(db, interview)
    proposals = _ensure_recalibration_proposal(db, interview, outcome=outcome, source=source, reason=reason, user_id=user_id)
    result["application"] = application_result
    result["recalibration_proposals"] = proposals
    return result


def _ensure_recalibration_proposal(db: Session, interview: Interview, outcome: str = "UNKNOWN", source: str = "", reason: str = "", user_id: str | None = None) -> list[dict[str, Any]]:
    proposal = db.scalar(select(ApplicationRecalibrationRun).where(ApplicationRecalibrationRun.interview_id == interview.id).order_by(ApplicationRecalibrationRun.created_at.desc()))
    if not proposal:
        proposal = ApplicationRecalibrationRun(
            application_id=interview.application_id,
            ...
            profile_id=interview.profile_id,
            status="suggested",
            observed_data_json={"interview_id": interview.id, "outcome": outcome, "outcome_source_known": bool(source), "reason_provided": bool(reason)},
            user_interpretation_json={"note": "Outcome source and user interpretation remain separate."},
            ai_interpretation_json={"note": "This is a proposal only; no profile or career hypothesis was changed."},
            suggestions_json=_reflection_suggestions({"weak_answers": [reason] if reason else [], "additional_evidence_needed": []}),
            roadmap_changes_require_confirmation=True,
            accepted_by_user=False,
            user_decision="PENDING",
            before_state_json={"career_hypothesis": "unchanged", "evidence_passport": "unchanged", "human_potential_map": "unchanged", "roadmap": "unchanged"},
            after_state_json={},
            source_label=source or "Interview outcome",
            limitation="One interview outcome provides limited evidence and does not determine career fit.",
            demo_marker=interview.demo_marker,
            version="interview-recalibration-v2",
        )
        db.add(proposal)
    else:
        proposal.observed_data_json = {**(proposal.observed_data_json or {}), "outcome": outcome, "outcome_source_known": bool(source), "reason_provided": bool(reason)}
        if source:
            proposal.source_label = source
        if reason:
            proposal.user_interpretation_json = {**(proposal.user_interpretation_json or {}), "recorded_reason": reason}
    db.commit()
    return [recalibration_proposal_public(proposal)]


def decide_interview_recalibration(db: Session, proposal: ApplicationRecalibrationRun, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    decision = str(payload.get("decision") or "").strip().lower()
    if decision not in {"accept", "edit", "reject"}:
        raise ValueError("Recalibration decision must be accept, edit, or reject.")
    proposal.user_decision = decision.upper()
    proposal.accepted_by_user = decision in {"accept", "edit"}
    proposal.status = "accepted" if decision in {"accept", "edit"} else "rejected"
    proposal.after_state_json = {
        "proposed_changes": _limited_list(payload.get("proposed_changes"), 20) if isinstance(payload.get("proposed_changes"), list) else proposal.suggestions_json or [],
        "profile_mutated": False,
        "user_edited": decision == "edit",
    } if proposal.accepted_by_user else {}
    if decision == "edit" and isinstance(payload.get("edited_suggestions"), list):
        proposal.suggestions_json = _limited_list(payload.get("edited_suggestions"), 20)
    proposal.ai_interpretation_json = {**(proposal.ai_interpretation_json or {}), "user_decision_recorded": True, "roadmap_mutated": False}
    db.commit()
    return recalibration_proposal_public(proposal)


def create_decision_journal_hook(db: Session, interview: Interview, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    source_type = str(payload.get("source_type") or "interview_outcome").strip().lower()
    if source_type not in {"interview_outcome", "offer_decision", "recalibration_decision"}:
        raise ValueError("Unsupported decision journal source type.")
    row = CareerDecisionJournalEntry(
        profile_id=interview.profile_id,
        user_id=user_id or interview.user_id,
        title=_clean_text(payload.get("title") or f"Interview decision: {interview.role}", 255),
        decision_type=source_type,
        status="active",
        decision_summary=_clean_text(payload.get("decision_summary") or "Review this interview outcome and record a human decision.", 8000),
        context=_clean_text(payload.get("context") or f"Interview stage: {_stage_label(interview.stage_type)}; outcome: {interview.interview_result}.", 8000),
        selected_option=_clean_text(payload.get("selected_option") or "", 255),
        options_json=_limited_list(payload.get("options"), 12),
        assumptions_json=_limited_list(payload.get("assumptions"), 20),
        evidence_links_json=_limited_list(payload.get("evidence_links"), 20),
        source_attributions_json=_limited_list(payload.get("source_attributions"), 32),
        system_suggestions_json=_limited_list(payload.get("system_suggestions"), 16),
        ai_explanations_json=_limited_list(payload.get("ai_explanations"), 16),
        evidence_observations_json=_limited_list(payload.get("evidence_observations"), 24),
        adviser_inputs_json=_limited_list(payload.get("adviser_inputs"), 24),
        user_reasoning=_clean_text(payload.get("user_reasoning") or "", 8000),
        interview_id=interview.id,
        job_analysis_id=interview.job_analysis_id,
        application_id=interview.application_id,
    ...
    db.add(
        CareerDecisionJournalVersion(
            ...
            snapshot_json={
                ...
                "application_id": row.application_id,
                "evidence_links": row.evidence_links_json or [],
                "evidence_observations": row.evidence_observations_json or [],
                "ai_explanations": row.ai_explanations_json or [],
                "system_suggestions": row.system_suggestions_json or [],
                "user_reasoning": row.user_reasoning,
                "roadmap_mutated": False,
            },
            change_reason="Created from Interview Journey by explicit user action.",
        )
    )
    db.commit()
    return {"id": row.id, "profile_id": row.profile_id, "title": row.title, "decision_type": row.decision_type, "application_id": row.application_id, "interview_id": interview.id, "status": row.status, "version_number": row.version_number, "roadmap_mutation_allowed": False, "created_at": row.created_at.isoformat()}
...
def create_offer_review(db: Session, profile: Profile, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    if app and interview and interview.application_id != app.id:
        raise ValueError("Offer review application and interview must refer to the same application.")
    actual_linked_offer = bool(app and payload.get("offer_confirmed", False))
    source = "linked_offer" if actual_linked_offer else "linked_application_comparison" if app else "manual_hypothetical"
    ...
    row = OfferReview(
        ...
        source=source,
        user_confirmed=bool(payload.get("offer_confirmed", False)),
        demo_marker=_demo_for_profile(profile),
        comparison_snapshot_json={
            "confirmed_offer_facts": review.get("confirmed_offer_facts", []),
            "missing_information": review.get("missing_information", []),
            "user_priorities": _limited_list(payload.get("user_priorities"), 12),
            "captured_at": _now().isoformat(),
        },
    )
    db.add(row)
...
def update_offer_review(db: Session, row: OfferReview, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row.comparison_snapshot_json = {
        "confirmed_offer_facts": row.review_json.get("confirmed_offer_facts", []),
        "missing_information": row.review_json.get("missing_information", []),
...
def compare_offer_reviews(db: Session, profile: Profile, payload: dict[str, Any]) -> dict[str, Any]:
    ids = [str(item) for item in _limited_list(payload.get("offer_review_ids") or payload.get("review_ids"), 6)]
    if len(ids) < 2:
        raise ValueError("At least two offer reviews are required for comparison.")
    rows = [require_offer_review(db, item, profile) for item in ids]
    priorities = [str(item) for item in _limited_list(payload.get("user_priorities"), 12)] or [str(item) for row in rows for item in (row.user_priorities_json or [])]
    comparison = []
    for row in rows:
        item = row.offer_items_json or {}
        priority_rows = []
        for priority in priorities:
            lower = priority.lower()
            matched_fields = [field for field, value in item.items() if value not in {None, ""} and any(token in f"{field} {value}".lower() for token in re.findall(r"[a-z0-9]+", lower))]
            priority_rows.append({"priority": priority, "status": "known_match" if matched_fields else "unknown_or_not_matched", "fields": matched_fields})
        comparison.append({
            "id": row.id,
            "organisation": row.organisation,
            "role": row.role,
            "confirmed_offer_facts": row.review_json.get("confirmed_offer_facts", []),
            "missing_information": row.review_json.get("missing_information", []),
            "priority_assessment": priority_rows,
            "tradeoffs": row.review_json.get("acceptance_considerations", []),
        })
    return {
        "offers": comparison,
        "user_priorities": priorities,
        "winner": None,
        "winner_policy": "No opaque universal winner; compare explicit user priorities and visible trade-offs.",
        "limitations": ["Unknown offer fields remain unknown.", "Legal, tax, pension and financial confirmation should come from official sources."],
        "source": "user-provided offer facts",
    }
...
def offer_review_public(row: OfferReview) -> dict[str, Any]:
    review_type = "ACTUAL_LINKED_OFFER" if row.application_id and row.source == "linked_offer" else "LINKED_APPLICATION_COMPARISON" if row.application_id else "MANUAL_HYPOTHETICAL"
    return {
        "id": row.id,
        "profile_id": row.profile_id,
        "application_id": row.application_id,
        ...
        "actual_offer_confirmed": review_type == "ACTUAL_LINKED_OFFER",
        "user_confirmed": row.user_confirmed,
        "demo_marker": row.demo_marker,
        "version_number": row.version_number,
        "comparison_snapshot": row.comparison_snapshot_json or {},
        "safety_labels": ["Information to verify", "Potential consideration", "Official confirmation recommended"],
        "created_at": row.created_at.isoformat(),
        "updated_at": row.updated_at.isoformat(),
    }
...
def seed_demo_interview_journey(db: Session, profile: Profile) -> None:
    ...
    if not analysed_app:
        return
    scenario_title = "Human-Centred AI Product Strategist"
    ...
    analysed_app.title = scenario_title
    analysed_app.organisation = "OrganicAI Demo Studio"
    if analysed_app.job_analysis_id:
        analysis = db.get(JobAnalysis, analysed_app.job_analysis_id)
        if analysis:
            analysis.title = scenario_title
            analysis.organisation = analysed_app.organisation
            analysis.user_confirmed = True
            existing_requirements = db.scalars(select(JobRequirement).where(JobRequirement.analysis_id == analysis.id).order_by(JobRequirement.order_index)).all()
            for index, (label, skill_id) in enumerate(scenario_requirements):
                requirement = existing_requirements[index] if index < len(existing_requirements) else JobRequirement(analysis_id=analysis.id, profile_id=profile.id, order_index=index)
                requirement.requirement_text = label
                requirement.requirement_category = "skills"
                requirement.requirement_type = "mandatory" if index < 4 else "preferred"
                requirement.normalised_skill_id = skill_id
                requirement.user_confirmation_state = "confirmed"
                requirement.status = "active"
                if requirement not in db:
                    db.add(requirement)
            db.flush()
    stages = [
        ("hiring_manager", "2026-07-25T10:00:00"),
    ...
    for index, (stage, scheduled) in enumerate(stages, 1):
        payload = {
            ...
            "participants": [{"role": "hiring manager" if stage == "hiring_manager" else "technical lead" if stage == "technical" else "director"}],
            "source": "demo",
    ...
    structured_requirement = db.scalar(select(JobRequirement).where(JobRequirement.analysis_id == first.job_analysis_id, JobRequirement.requirement_text == "Structured problem solving")) if first.job_analysis_id else None
    if not db.scalar(select(InterviewQuestion).where(InterviewQuestion.interview_id == first.id, InterviewQuestion.question_text == "Tell me about a time you converted ambiguous requirements into a structured recommendation.")):
        db.add(
            InterviewQuestion(
                interview_id=first.id,
                profile_id=profile.id,
                application_id=first.application_id,
                job_analysis_id=first.job_analysis_id,
                category="situational",
                question_type="situational",
                stage="hiring_manager",
                question_text="Tell me about a time you converted ambiguous requirements into a structured recommendation.",
                why_it_may_be_asked="Plausible practice question linked to the confirmed requirement Structured problem solving; it is not a prediction of employer wording.",
                related_job_requirement_id=structured_requirement.id if structured_requirement else None,
                related_job_requirement="Structured problem solving",
                related_evidence_json=[{"source": "Career Experiment #CE-demo-structured-recommendation", "support_state": "PARTIALLY_SUPPORTED"}],
                answer_objective="Use a concise STAR answer and separate the confirmed project evidence from transferable reasoning.",
                source_type="deterministic_demo_scenario",
                origin="deterministic",
                question_status="kept",
                saved_by_user=True,
            )
        )
        db.flush()
    ...
    record_interview_outcome(
        db,
        first,
        {"outcome": "NEXT_STAGE", "source": "Demo-recorded outcome", "reason": "Demo workflow outcome."},
        profile.user_id,
    )
