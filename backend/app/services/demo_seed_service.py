...
def restore_demo(db: Session) -> tuple[User,Profile,Roadmap]:
    ...
    delete_learning_for_profiles(db,profile_ids+[DEMO_PROFILE_ID])
    delete_assessment_for_profiles(db,profile_ids+[DEMO_PROFILE_ID])
...
def ensure_demo(db:Session, reset:bool=False):
    ...
    if profile and roadmap and fear_count >= 2 and conversation_count and assessment_count and learning_count:
