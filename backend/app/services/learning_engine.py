...
from app.models.assessment import AssessmentResponse, AssessmentSession, CareerMatch, CareerRoleTemplate, SkillEvidence, SkillsInventory
from app.models.career_resilience import CareerHypothesis, SkillEvidenceConfidence
...
class CuratedCatalogueProvider:
    ...
    def check_availability(self, external_id: str) -> ResourceAvailability:
        ...
        return ResourceAvailability(bool(row and row.active), "curated_catalogue", utc_now_naive())
...
class DisabledExternalProviderAdapter:
    ...
    def check_availability(self, external_id: str) -> ResourceAvailability:
        return ResourceAvailability(False, "external_api_disabled", utc_now_naive(), "External API access is disabled.")
...
def practically_verified_skill_ids(db: Session, profile_id: str) -> set[str]:
    """Read authoritative practical evidence without altering the source inventory claim."""
    return set(
        db.scalars(
            select(SkillEvidenceConfidence.skill_id)
...
def create_skill_gap_analysis(db: Session, profile: Profile, career_match_id: str | None = None) -> dict[str, Any]:
    ...
    practically_verified_skills = practically_verified_skill_ids(db, profile.id)
    ...
    for index, skill_id in enumerate(required_skills):
        ...
        evidence_status = "practically_verified" if skill_id in practically_verified_skills else (inventory.evidence_status if inventory else "self_reported")
...
def latest_gap_analysis(db: Session, profile_id: str, career_match_id: str | None = None) -> dict[str, Any]:
    statement = select(SkillGapAnalysis).where(SkillGapAnalysis.profile_id == profile_id, SkillGapAnalysis.status == "ready")
    if career_match_id:
...
def score_alignment(
    ...
    if resource.last_verified_at:
        age_days = (utc_now_naive() - resource.last_verified_at).days
...
def recommendation_public(
    ...
    match = db.get(CareerMatch, row.career_match_id) if row.career_match_id else None
    hypothesis = db.scalar(
        select(CareerHypothesis).where(
            CareerHypothesis.profile_id == row.profile_id,
            CareerHypothesis.career_match_id == row.career_match_id,
            CareerHypothesis.status == "active",
        )
    ) if row.career_match_id else None
    return {
        "id": row.id,
        "run_id": row.run_id,
        "profile_id": row.profile_id,
        ...
        "career_direction": {
            "label": hypothesis.title if hypothesis else match.title if match else "Selected career direction",
            "source": "Career Hypothesis" if hypothesis else "Career direction",
            "version": hypothesis.current_version_number if hypothesis else None,
        },
...
def recommendation_run_public(db: Session, run_id: str) -> dict[str, Any]:
    ...
    recommendations = [recommendation_public(db, row, resources[row.learning_resource_id], factors_by_rec[row.id], gaps.get(row.skill_gap_item_id), objectives.get(row.learning_objective_id)) for row in rows]
...
def latest_recommendations(db: Session, profile_id: str, career_match_id: str | None = None) -> dict[str, Any]:
    statement = (
        select(LearningRecommendationRun)
        .join(SkillGapAnalysis, SkillGapAnalysis.id == LearningRecommendationRun.skill_gap_analysis_id)
        .where(LearningRecommendationRun.profile_id == profile_id, SkillGapAnalysis.status == "ready")
    )
    if career_match_id:
...
def cache_provider_failure(db: Session, provider_name: str, cache_key: str, error_message: str) -> None:
    ...
    db.add(
        ExternalProviderCache(
            ...
            expires_at=utc_now_naive() + timedelta(seconds=settings.learning_resource_cache_ttl_seconds),
        )
    )
    db.flush()
