...
def diverse(items:list[dict],max_per_category=1):
    ...
    for item in sorted(items,key=lambda x:x["relevance_score"],reverse=True):
        ...
        if any(len(keywords(item["title"])&keywords(old["title"]))>=2 for old in output):continue
...
async def generate_recommendations(db:Session,profile:Profile,user_id:str|None,categories:list[str]|None,force:bool)->dict:
    started=perf_counter();t=perf_counter();ctx=build_recommendation_context(db,profile);context_ms=int((perf_counter()-t)*1000);t=perf_counter();candidates=generate_rule_candidates(ctx,categories);candidate_ms=int((perf_counter()-t)*1000)
    previous=db.scalars(select(Recommendation).where(Recommendation.profile_id==profile.id)).all();blocked=[keywords(item.title) for item in previous if item.status=="rejected"]
    candidates=[item for item in candidates if not any(len(keywords(item["title"])&words)>=2 for words in blocked)]
    if force:
    ...
    for item in candidates:
        ...
        signals=[{"signal":str(signal),"source":"profile_feedback" if signal in ctx.get("confirmed_strengths",[]) else "diagnostic","weight":round(.82-index*.06,2)} for index,signal in enumerate((ctx.get("confirmed_strengths",[])+ctx.get("values",[])+ctx.get("learning_preferences",[]))[:4])]
        scored=score_candidate(item,ctx,rag);scored.update({"rag_sources":rag,"profile_signals":signals,"retrieval_metadata":{"query":query,"top_score":rag[0]["score"] if rag else 0,"chunks_used":len(rag),"threshold":.10},"reason":f"This fits the available signals: {', '.join(x['signal'] for x in signals[:3]) or ctx.get('primary_archetype','your profile')}."});enriched.append(scored)
    ...
    return {"profile_id":profile.id,"generation_id":generation_id,"recommendations":[public(item) for item in saved],"generated_at":datetime.utcnow().isoformat(),"context_summary":{"profile_signals_used":[x for x in ctx.get("confirmed_strengths",[])[:4]],"feedback_applied":ctx.get("feedback_applied",False),"rag_used":any(item.rag_sources_json for item in saved)},"metadata":{"context_build_ms":context_ms,"candidate_generation_ms":candidate_ms,"retrieval_ms":retrieval_ms,"llm_refinement_ms":0,"scoring_ms":scoring_ms,"persistence_ms":persistence_ms,"total_ms":int((perf_counter()-started)*1000),"pipeline":["Profile Context","Feedback Overrides","Candidate Generation","RAG Retrieval","Scoring","Diversity Filter","Explanation","User Feedback"]}}
