...
from app.models.career_resilience import CareerExperimentSession, CareerHypothesis
...
ADVISOR_PERMISSION_CODES = {"READ_ONLY": "View only", "COMMENT": "Comment", "PROPOSE_CHANGE": "Suggest changes"}
...
def _normalise_permission(value: Any) -> tuple[str, str]:
    raw = str(value or "View only").strip()
    if raw in ADVISOR_PERMISSION_CODES:
        return ADVISOR_PERMISSION_CODES[raw], raw
    aliases = {
        "VIEW_ONLY": "READ_ONLY",
        "VIEW ONLY": "READ_ONLY",
        "COMMENT": "COMMENT",
        "SUGGEST_CHANGES": "PROPOSE_CHANGE",
        "SUGGEST CHANGES": "PROPOSE_CHANGE",
        "PROPOSE_CHANGE": "PROPOSE_CHANGE",
    }
    code = aliases.get(raw.upper())
    if code:
        return ADVISOR_PERMISSION_CODES[code], code
    if raw in ADVISOR_PERMISSION_LEVELS:
        return raw, {"View only": "READ_ONLY", "Comment": "COMMENT", "Suggest changes": "PROPOSE_CHANGE"}.get(raw, "PROPOSE_CHANGE")
    raise ValueError("Unsupported adviser permission level.")


def _excluded_share_sections(selected: list[str]) -> list[str]:
    excluded = [name for name in sorted(ADVISOR_SHAREABLE_SECTIONS) if name not in selected]
    return excluded + sorted(SENSITIVE_EXCLUDED_SECTIONS)
...
def create_job_capture(db: Session, profile: Profile, payload: dict[str, Any], connection: BrowserExtensionConnection | None = None, user_id: str | None = None) -> dict[str, Any]:
    ...
    db.add(row)
    db.flush()
    # The requested action is a user intention, not confirmation. The review
    # screen must remain the only place that can create an authoritative Job
    # Analysis from browser content.
...
def capture_public(row: BrowserJobCapture | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "source_type": row.source_type or "BROWSER_CAPTURE",
        ...
        "status": row.status,
        "review_required": row.status in {"Captured", "Needs review"} and not row.job_analysis_id,
...
def archive_job_capture(db: Session, profile: Profile, capture_id: str, user_id: str | None = None) -> dict[str, Any]:
    row = db.get(BrowserJobCapture, capture_id)
    if not row or row.profile_id != profile.id:
        raise LookupError("Job capture not found")
    if row.job_analysis_id:
        raise ValueError("A capture linked to a Job Analysis cannot be removed from provenance; archive it from the Job Analyzer instead.")
    row.status = "Archived"
    row.updated_at = _now()
    _audit(db, profile.id, "job_capture_archived", "browser_job_capture", row.id, "user", user_id or profile.user_id or "")
    db.commit()
    return capture_public(row)
...
def confirm_job_capture(db: Session, profile: Profile, capture_id: str, payload: dict[str, Any] | None = None, user_id: str | None = None) -> dict[str, Any]:
    ...
    if row.status == "Archived":
        raise ValueError("Archived browser captures cannot be confirmed.")
    confirmed_text = _clean_text(payload.get("sanitised_text") or row.sanitised_text or row.selected_text, MAX_CAPTURED_TEXT_CHARS)
    if not confirmed_text:
        raise ValueError("Confirmed capture text is required before analysis.")
    ...
    row.confirmed_text = confirmed_text
    row.user_edited_text = confirmed_text != (row.sanitised_text or row.selected_text)
    row.sanitised_text = confirmed_text
    ...
    if payload.get("analyse") or row.requested_action == "save_and_analyse":
        analysis = create_job_analysis(
            ...
            profile,
            {
                ...
                "capture_id": row.id,
                "pasted_text": row.confirmed_text,
        ...
        if analysis_row:
            ...
            row.status = "Analysed"
            row.analysis_version = analysis_row.extraction_version
...
def advisor_share_public(db: Session, row: AdvisorShare, include_token: str | None = None, include_sections: bool = False) -> dict[str, Any]:
    ...
    audit_rows = db.scalars(select(InnovationAuditEvent).where(InnovationAuditEvent.target_id == row.id).order_by(InnovationAuditEvent.created_at.desc()).limit(30)).all()
    payload = {
        "id": row.id,
        "profile_id": row.profile_id,
        ...
        "permission_code": row.permission_code or "READ_ONLY",
        "allowed_sections": row.allowed_sections_json or [],
        "excluded_sections": row.excluded_sections_json or _excluded_share_sections(row.allowed_sections_json or []),
        "scope_version": row.version_number,
        "scope_snapshot": row.scope_snapshot_json or {},
        ...
        "audit_events": [{"event_type": item.event_type, "actor_type": item.actor_type, "created_at": item.created_at.isoformat()} for item in audit_rows],
        "limitations": [
            ...
            "Once an adviser has viewed shared information, OrganicAI Compass cannot prevent external copying or screenshots.",
        ],
        "share_preview": {
            "heading": "You are about to share:",
            "included_sections": row.allowed_sections_json or [],
            "excluded_sections": row.excluded_sections_json or _excluded_share_sections(row.allowed_sections_json or []),
...
def create_advisor_share(db: Session, profile: Profile, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    if role not in ADVISOR_ROLES:
        role = "Other"
    permission_level, permission_code = _normalise_permission(payload.get("permission_level"))
    ...
    minimum_actions = {"READ_ONLY": ["view"], "COMMENT": ["view", "comment"], "PROPOSE_CHANGE": ["view", "comment", "suggest_changes"]}[permission_code]
    actions = list(dict.fromkeys([*minimum_actions, *actions]))
    if permission_code == "READ_ONLY":
        actions = ["view"]
    token, token_hash = _make_token()
    ...
    row = AdvisorShare(
        ...
        excluded_sections_json=_excluded_share_sections(sections),
        scope_snapshot_json={"version": 1, "included_sections": sections, "excluded_sections": _excluded_share_sections(sections)},
    ...
    db.add(row)
    db.flush()
    _audit(db, profile.id, "advisor_share_created", "advisor_share", row.id, "user", user_id or profile.user_id or "", {"sections": sections, "excluded_sections": _excluded_share_sections(sections), "permission_level": permission_code, "scope_version": 1})
    db.commit()
...
def submit_advisor_comment(db: Session, token: str, payload: dict[str, Any], pin: str | None = None) -> dict[str, Any]:
    ...
    if target_type != "share" and target_type not in (share.allowed_sections_json or []):
        raise PermissionError("Adviser comments are limited to the sections included in this share.")
    ...
    proposal_type = _clean_text(payload.get("proposal_type") or "COMMENT", 80)
    proposal_payload = payload.get("proposal_payload") if isinstance(payload.get("proposal_payload"), dict) else {}
    if proposal_type == "COMMENT" and proposal_payload:
        proposal_type = "PROPOSE_CHANGE"
    comment = AdvisorComment(
        ...
        target_version=_clean_text(payload.get("target_version") or "", 120),
...
def advisor_comment_public(row: AdvisorComment) -> dict[str, Any]:
    return {
        ...
        "is_proposal": row.proposal_type != "COMMENT",
...
def respond_to_advisor_comment(db: Session, profile: Profile, comment_id: str, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    if decision in {"edit", "edited"}:
        edited_payload = payload.get("proposal_payload")
        if not isinstance(edited_payload, dict):
            raise ValueError("Editing an adviser proposal requires proposal_payload.")
        row.proposal_payload_json = {**(row.proposal_payload_json or {}), "owner_edit": edited_payload}
        row.status = "pending"
        audit_decision = "edited"
    elif decision in {"accepted", "rejected", "pending"}:
        row.status = decision
        audit_decision = decision
    else:
        raise ValueError("Advisor comment response must be accepted, rejected, pending, or edited.")
    row.user_response = _clean_text(payload.get("user_response") or "", 2000)
    row.version_number += 1 if audit_decision == "edited" else 0
    row.resolved_at = _now() if audit_decision in {"accepted", "rejected"} else None
    row.updated_at = _now()
    _audit(db, profile.id, f"advisor_comment_{audit_decision}", "advisor_comment", row.id, "user", user_id or profile.user_id or "", {"profile_mutated": False, "evidence_mutated": False, "proposal_version": row.version_number})
    db.commit()
...
def career_role_public(row: CareerRoleProfile) -> dict[str, Any]:
    ...
    return {
        ...
        "profile": profile,
        "status": row.status,
        "curated_status": "CURATED REFERENCE",
        "market_data_status": "NOT LIVE MARKET DATA",
        "limitations": profile.get("known_uncertainties", ["This is curated reference content, not a live labour-market feed."]),
...
def list_career_roles(db: Session, family: str | None = None, include_archived: bool = False, search: str | None = None, skill: str | None = None) -> list[dict[str, Any]]:
    ...
    skill_needle = str(skill or "").strip().lower()
    output: list[dict[str, Any]] = []
    for row in rows:
        public = career_role_public(row)
        if needle:
            searchable = " ".join([row.title, row.slug, row.career_family, row.summary, *[str(item) for item in (row.aliases_json or [])]]).lower()
            if needle not in searchable:
                continue
        if skill_needle:
            skills = " ".join([str(item) for item in (row.profile_json or {}).get("technical_skills", []) + (row.profile_json or {}).get("transferable_skills", [])]).lower()
            if skill_needle not in skills:
                continue
...
def journal_snapshot(row: CareerDecisionJournalEntry) -> dict[str, Any]:
    return {
        ...
        "source_attributions": row.source_attributions_json or [],
        ...
        "adviser_inputs": row.adviser_inputs_json or [],
...
def _validate_journal_links(db: Session, profile: Profile, payload: dict[str, Any]) -> None:
    for key, model, label in [
        ("job_analysis_id", JobAnalysis, "Job Analysis"),
        ("application_id", JobApplication, "Application"),
        ("linked_experiment_id", CareerExperimentSession, "Career Experiment"),
        ("interview_id", Interview, "Interview"),
    ]:
        value = payload.get(key)
        if value:
            row = db.get(model, value)
            if not row or getattr(row, "profile_id", None) != profile.id:
                raise PermissionError(f"Linked {label} does not belong to this profile.")
    for comment_id in _limited_list(payload.get("adviser_comment_ids"), 20):
        row = db.get(AdvisorComment, str(comment_id))
        if not row or row.profile_id != profile.id:
            raise PermissionError("Linked adviser input does not belong to this profile.")


def create_journal_entry(db: Session, profile: Profile, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    _validate_journal_links(db, profile, payload)
    row = CareerDecisionJournalEntry(
        ...
        uncertainty_json=payload.get("uncertainty") if isinstance(payload.get("uncertainty"), dict) else {},
        confidence=_clean_text(payload.get("confidence") or "", 80),
        reversibility=_clean_text(payload.get("reversibility") or "", 80),
        ...
        user_reasoning=_clean_text(payload.get("user_reasoning") or "", MAX_JOURNAL_TEXT_CHARS),
        adviser_comment_ids_json=_limited_list(payload.get("adviser_comment_ids"), 20),
        career_slug=payload.get("career_slug"),
        linked_experiment_id=payload.get("linked_experiment_id"),
        interview_id=payload.get("interview_id"),
        job_analysis_id=payload.get("job_analysis_id"),
        application_id=payload.get("application_id"),
        privacy_scope=payload.get("privacy_scope") or "private",
        review_date=payload.get("review_date"),
        later_outcome=_clean_text(payload.get("later_outcome") or "", MAX_JOURNAL_TEXT_CHARS),
        lessons_learned=_clean_text(payload.get("lessons_learned") or "", MAX_JOURNAL_TEXT_CHARS),
...
def get_journal_entry(db: Session, profile: Profile, entry_id: str) -> dict[str, Any]:
    ...
    _validate_journal_links(db, profile, journal_snapshot(row))
    return journal_public(db, row, include_versions=True)
...
def update_journal_entry(db: Session, profile: Profile, entry_id: str, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    for key in ["linked_experiment_id", "interview_id"]:
        if key in payload:
            setattr(row, key, payload.get(key))
    for key, limit in [("confidence", 80), ("reversibility", 80), ("user_reasoning", MAX_JOURNAL_TEXT_CHARS), ("later_outcome", MAX_JOURNAL_TEXT_CHARS), ("lessons_learned", MAX_JOURNAL_TEXT_CHARS)]:
        if key in payload and payload[key] is not None:
            setattr(row, key, _clean_text(payload[key], limit))
    if "uncertainty" in payload:
        row.uncertainty_json = payload.get("uncertainty") if isinstance(payload.get("uncertainty"), dict) else {}
    for source, target, limit in [
        ("source_attributions", "source_attributions_json", 32),
        ("system_suggestions", "system_suggestions_json", 16),
        ("ai_explanations", "ai_explanations_json", 16),
        ("evidence_observations", "evidence_observations_json", 24),
        ("adviser_inputs", "adviser_inputs_json", 24),
    ]:
        if source in payload:
            setattr(row, target, _limited_list(payload.get(source), limit))
...
def record_journal_outcome(db: Session, profile: Profile, entry_id: str, payload: dict[str, Any], user_id: str | None = None) -> dict[str, Any]:
    ...
    row.later_outcome = _clean_text(payload.get("later_outcome") or payload.get("actual_outcome") or "", MAX_JOURNAL_TEXT_CHARS)
    row.lessons_learned = _clean_text(payload.get("lessons_learned") or "", MAX_JOURNAL_TEXT_CHARS)
...
def seed_demo_innovation_extension(db: Session, user_id: str, profile: Profile) -> None:
    ...
    capture = BrowserJobCapture(
        ...
        confirmed_text="Mandatory requirements include UX design, responsible AI, accessibility, evaluation and stakeholder communication.",
        confirmed_at=_now(),
    ...
    db.flush()
    try:
        analysis = create_job_analysis(db, profile, {"input_type": "browser_capture_confirmed", "capture_id": capture.id, "pasted_text": capture.confirmed_text, "title": capture.detected_title, "organisation": capture.detected_employer})
        analysis_row = db.get(JobAnalysis, analysis["id"])
        if analysis_row:
    ...
    share = AdvisorShare(
        ...
        permission_code="PROPOSE_CHANGE",
        allowed_sections_json=["Career Hypotheses", "Evidence Passport", "Job Analysis", "Career Decision Journal"],
        excluded_sections_json=_excluded_share_sections(["Career Hypotheses", "Evidence Passport", "Job Analysis", "Career Decision Journal"]),
        scope_snapshot_json={"version": 1, "included_sections": ["Career Hypotheses", "Evidence Passport", "Job Analysis", "Career Decision Journal"], "excluded_sections": _excluded_share_sections(["Career Hypotheses", "Evidence Passport", "Job Analysis", "Career Decision Journal"])},
