...
logger = logging.getLogger("organicai.elevenlabs")
...
def _safe_upstream_message(response: httpx.Response) -> str:
    """Return a bounded diagnostic without ever logging credentials or tokens."""
    ...
    if isinstance(payload, dict):
        payload = payload.get("detail") or payload.get("message") or payload.get("error") or "upstream request failed"
        if isinstance(payload, dict):
            payload = payload.get("message") or payload.get("detail") or payload.get("code") or "upstream request failed"
    return " ".join(str(payload).split())[:300]


def _log_token_failure(*, status: int | None, error_type: str, message: str, agent_id: str | None) -> None:
    logger.warning(
        "ElevenLabs token request failed status=%s error_type=%s message=%s agent_id_suffix=%s",
        status if status is not None else "transport",
        error_type,
        " ".join(message.split())[:300],
        (agent_id or "")[-6:] or "none",
    )
...
async def get_conversation_token(*, participant_name: str) -> dict[str, str]:
    ...
    try:
        ...
        _log_token_failure(status=None, error_type=type(error).__name__, message=str(error), agent_id=settings.elevenlabs_agent_id)
