...
from app.services.rag_service import format_sources_for_prompt, search_knowledge_base
...
async def generate_coach_response(
    ...
    all_sources = await search_knowledge_base(message)
    threshold = 0.10
    sources = [source for source in all_sources if source.score >= threshold]
    ...
    return {
        ...
        "retrieval_status": {"query": message, "chunks_considered": len(all_sources), "chunks_used": len(sources), "top_score": round(all_sources[0].score, 4) if all_sources else 0, "threshold": threshold},
