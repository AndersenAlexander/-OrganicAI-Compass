...
def normalize_legacy(db: Session, roadmap: Roadmap) -> None:
    ...
    for horizon in HORIZONS:
        ...
        for index, item in enumerate(raw if isinstance(raw, list) else []):
            ...
            db.add(RoadmapAction(roadmap_id=roadmap.id,profile_id=roadmap.profile_id,user_id=roadmap.user_id,recommendation_id=item.get("recommendation_id"),horizon=horizon,title=title,description=str(item.get("description") or ""),reason="Carried forward from your existing roadmap.",first_step="Choose a small first step and schedule it.",success_criteria="Record a short outcome.",estimated_minutes=45,priority=index+1,source_type="recommendation" if item.get("recommendation_id") or item.get("source")=="recommendation" else "legacy_roadmap"))
    db.flush()
...
def snapshot(db, roadmap, reason):
    ...
    roadmap.data={**roadmap.data,"version":version,"updated_at":datetime.utcnow().isoformat()}
...
def propose_recalibration(db, roadmap):
    ...
    if len(unstarted)>=3:
        ...
        rules.append("Three unstarted seven-day actions → postpone lower-priority actions")
    if checkins and checkins[0].energy_level and checkins[0].energy_level<=2:
        ...
        rules.append("Low energy → reduce the next action scope")
