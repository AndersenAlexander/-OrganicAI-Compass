...
METHODOLOGY_NOTE = (
    "Personality and work-style items are original, non-clinical Big Five-inspired prompts "
    "created for this research prototype. They are not copied from proprietary tests and are "
    "not intended for diagnosis, hiring, or clinical use."
)
...
def assessment_definition() -> dict[str, Any]:
    ...
    modules = [
        ...
        {"id": "personality_work_style", "title": "Personality, Work Style & Career Fit", "description": "Non-clinical current tendencies and preferences.", "order": 3},
...
def compute_role_match(template: dict[str, Any], scores: dict[str, Any]) -> MatchComputation:
    ...
    supporting = []
    if interest_match >= 65:
        supporting.append("Interest profile suggests meaningful activity alignment.")
    if skills_match >= 60:
        supporting.append("Several current or transferable skills already support this direction.")
    if aligned_values:
    ...
    return MatchComputation(components, missing, transferable, supporting, conflicting)
...
def create_career_matches(db: Session, session: AssessmentSession, profile: Profile, scores: dict[str, Any]) -> list[CareerMatch]:
    ...
    augment = CareerMatch(
        ...
        explanation="This option starts with the user's current context and tests practical AI support before reskilling.",
        ...
        source_metadata_json={"source_type": "current_profession_augmentation", "version": ROLE_CATALOGUE_VERSION},
        assessment_version=session.assessment_version,
        scoring_version=session.scoring_version,
        demo_marker=session.demo_marker,
    )
    ...
    for role_row in role_rows:
        ...
        score = weighted_alignment(computation.components)
        ...
        row = CareerMatch(
            ...
            explanation=f"Your current answers suggest this role family may have {alignment_label(score).lower()}. Test the direction before making a major transition.",
            ...
            next_step=(role_row.typical_transition_path_json or ["Review real job descriptions"])[0],
            transition_difficulty=difficulty,
            time_horizon=time_horizon_for(difficulty),
            assumptions_json=["Role templates are curated prototype data, not universal labour-market truth.", "Self-reported skills are not treated as objectively verified."],
            limitations_json=["Personality and interests are exploration signals, not proof of suitability.", "Employment outcomes depend on market, evidence, practice, and timing."],
            source_metadata_json=role_row.source_metadata_json,
            assessment_version=session.assessment_version,
            scoring_version=session.scoring_version,
            demo_marker=session.demo_marker,
        )
...
def career_matches_for_profile(db: Session, profile_id: str, include_rejected: bool = False) -> list[dict[str, Any]]:
    query = select(CareerMatch).where(CareerMatch.profile_id == profile_id)
    if not include_rejected:
        query = query.where(CareerMatch.status != "rejected")
    rows = db.scalars(query.order_by(CareerMatch.category, CareerMatch.alignment_score.desc())).all()
