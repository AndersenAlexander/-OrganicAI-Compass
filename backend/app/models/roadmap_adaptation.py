...
class RoadmapAction(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            name="uq_roadmap_actions_career_experiment_session_id",
        ),
    )
    ...
    # These are explicit, immutable source identifiers. They intentionally do
    # not use foreign keys because CareerExperimentSession already references
    # RoadmapAction and demo-data reset removes both aggregate trees in stages.
    career_experiment_session_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    career_hypothesis_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
