...
class CareerExperimentSession(Base):
    ...
    evidence_gap_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
...
class CareerExperimentResult(Base):
    ...
    # The same submitted artefact can be reviewed repeatedly from the UI.  Keep
    # one durable deterministic result for that exact evaluation input.
    ...
    evidence_proposal_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now_naive)
...
class CareerHypothesis(Base):
    ...
    # A stable direction identity is intentionally separate from the display
    # title.  A single direction can be produced by both the Human Diagnostic
    # and a later deep-dive assessment with different snapshots.
    canonical_direction_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    ...
    fit_band: Mapped[str] = mapped_column(String(80), default="Insufficient evidence")
    based_on_diagnostic_version: Mapped[str] = mapped_column(String(80), default="")
    based_on_profile_version: Mapped[str] = mapped_column(String(80), default="")
    supporting_signals_json: Mapped[list] = mapped_column(JSON, default=list)
    caution_signals_json: Mapped[list] = mapped_column(JSON, default=list)
    missing_evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    conflicting_evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    source_breakdown_json: Mapped[dict] = mapped_column(JSON, default=dict)
    market_limitations_json: Mapped[list] = mapped_column(JSON, default=list)
    support_limitations_json: Mapped[list] = mapped_column(JSON, default=list)
    explanation_json: Mapped[dict] = mapped_column(JSON, default=dict)
    user_decision_state: Mapped[str] = mapped_column(String(40), default="UNREVIEWED", index=True)
    source_metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    demo_marker: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
...
class CareerEvidenceGap(Base):
    __tablename__ = "career_evidence_gaps"
    ...
    hypothesis_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    career_match_id: Mapped[str | None] = mapped_column(ForeignKey("career_matches.id"), nullable=True, index=True)
    skill_id: Mapped[str] = mapped_column(String(120), default="", index=True)
    capability_label: Mapped[str] = mapped_column(String(180), default="")
    gap_kind: Mapped[str] = mapped_column(String(40), default="evidence_gap", index=True)
    reason: Mapped[str] = mapped_column(Text, default="")
    current_evidence_status: Mapped[str] = mapped_column(String(40), default="MISSING", index=True)
    importance: Mapped[float] = mapped_column(Float, default=0)
    recency_issue: Mapped[str] = mapped_column(Text, default="")
    ...
    source_metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    version: Mapped[str] = mapped_column(String(60), default="career-evidence-gap-v1")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now_naive)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now_naive, onupdate=utc_now_naive)


class CareerEvidenceProposal(Base):
    __tablename__ = "career_evidence_proposals"
    ...
    hypothesis_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    evidence_gap_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    experiment_session_id: Mapped[str | None] = mapped_column(ForeignKey("career_experiment_sessions.id"), nullable=True, index=True)
    experiment_result_id: Mapped[str | None] = mapped_column(ForeignKey("career_experiment_results.id"), nullable=True, index=True)
    source_type: Mapped[str] = mapped_column(String(60), default="EXPERIMENT", index=True)
    category: Mapped[str] = mapped_column(String(80), default="project_evidence")
    capability_id: Mapped[str] = mapped_column(String(120), default="", index=True)
    capability_label: Mapped[str] = mapped_column(String(180), default="")
    title: Mapped[str] = mapped_column(String(255), default="")
    description: Mapped[str] = mapped_column(Text, default="")
    artifact_reference: Mapped[str] = mapped_column(Text, default="")
    expected_evidence_gain: Mapped[str] = mapped_column(String(80), default="Moderate")
    actual_evidence_gain: Mapped[str] = mapped_column(String(80), default="Unknown")
    verification_state: Mapped[str] = mapped_column(String(60), default="PROVISIONAL", index=True)
    relevance: Mapped[str] = mapped_column(String(80), default="Pending user review")
    recency: Mapped[str] = mapped_column(String(80), default="Dated at acceptance")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    status: Mapped[str] = mapped_column(String(40), default="PENDING_REVIEW", index=True)
    user_confirmed: Mapped[bool] = mapped_column(Boolean, default=False)
    user_edit_json: Mapped[dict] = mapped_column(JSON, default=dict)
