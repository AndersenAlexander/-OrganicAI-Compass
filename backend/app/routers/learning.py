...
def require_recommendation(db: Session, recommendation_id: str, user: User | None) -> LearningRecommendation:
    ...
    if user and item.user_id and item.user_id != user.id:
        raise HTTPException(403, "Learning recommendation does not belong to the current user")
    return item
...
def recommendation_details(db: Session, recommendation: LearningRecommendation) -> dict[str, Any]:
    ...
    payload = recommendation_public(recommendation, resource, factor_rows, gap, objective)
...
async def post_learning_path_progress(
    ...
    if user and path and path.user_id and path.user_id != user.id:
        raise HTTPException(403, "Learning path item does not belong to the current user")
