...
def require_profile(db: Session, profile_id: str, user: User | None) -> Profile:
    if profile_id in {"undefined", "null", ""}:
        raise HTTPException(422, "A valid profile id is required.")
    profile = db.get(Profile, profile_id)
    if not profile:
        raise HTTPException(404, "Profile not found")
    if user and profile.user_id and profile.user_id != user.id:
        raise HTTPException(403, "Profile does not belong to the current user")
    return profile
...
def require_session(db: Session, session_id: str, user: User | None) -> AssessmentSession:
    ...
    if user and session.user_id and session.user_id != user.id:
        raise HTTPException(403, "Assessment session does not belong to the current user")
    return session
...
def require_match(db: Session, match_id: str, user: User | None) -> CareerMatch:
    ...
    if not match:
        raise HTTPException(404, "Career match not found")
    if user and match.user_id and match.user_id != user.id:
        raise HTTPException(403, "Career match does not belong to the current user")
    return match
...
async def create_session(
    ...
    db.commit()
    db.refresh(session)
    return {"session": session_public(session), "definition": definition_for_mode(payload.mode), "disclaimer": DISCLAIMER}
...
async def current_session(
    ...
    if not session:
        return {"session": None, "definition": definition_for_mode("quick"), "disclaimer": DISCLAIMER}
    return {"session": session_public(session, include_responses=True, db=db), "definition": definition_for_mode(session.mode), "disclaimer": DISCLAIMER}
