...
from app.services.research_readiness import research_readiness
router=APIRouter();settings=get_settings()
def guard(user:User):
    readiness=research_readiness()
    if not readiness["ready"]:raise HTTPException(403,"Research configuration incomplete. Live research export is disabled.")
