...
class ExperimentSessionRequest(BaseModel):
    ...
    expected_evidence_gain: str = "Moderate"
...
class EvidenceProposalReviewRequest(BaseModel):
    decision: str
    note: str = ""
    reason: str = ""
...
class RoadmapConfirmationRequest(BaseModel):
    confirmed: bool = True
...
@router.get("/profiles/{profile_id}/career-hypotheses")
async def get_profile_hypotheses(
    profile_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> list[dict[str, Any]]:
    profile = require_profile(db, profile_id, user)
    return list_profile_hypotheses(db, profile)


@router.patch("/career-hypotheses/{hypothesis_id}/decision")
async def patch_hypothesis_decision(
    hypothesis_id: str,
    payload: HypothesisDecisionRequest,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> dict[str, Any]:
    row = db.get(CareerHypothesis, hypothesis_id)
    if not row:
        raise HTTPException(404, "Career hypothesis not found")
    profile = require_profile(db, row.profile_id, user)
    try:
        return set_hypothesis_decision(db, row, payload.state)
    except Exception as error:
        _handle(error)
        raise


@router.get("/profiles/{profile_id}/career-evidence-gaps")
async def get_profile_career_evidence_gaps(
    profile_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> dict[str, Any]:
    profile = require_profile(db, profile_id, user)
    try:
        return list_profile_evidence_gaps(db, profile)
    except Exception as error:
        _handle(error)
        raise


@router.get("/profiles/{profile_id}/evidence-proposals")
async def get_profile_evidence_proposals(
    profile_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> list[dict[str, Any]]:
    profile = require_profile(db, profile_id, user)
    return list_evidence_proposals(db, profile)


@router.post("/evidence-proposals/{proposal_id}/review")
async def post_evidence_proposal_review(
    proposal_id: str,
    payload: EvidenceProposalReviewRequest,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> dict[str, Any]:
    ...
    profile = require_profile(db, proposal.profile_id, user)
    try:
        return review_evidence_proposal(db, profile, proposal_id, payload.model_dump(), user.id if user else profile.user_id)
    except Exception as error:
        _handle(error)
        raise


@router.get("/profiles/{profile_id}/career-recalibration/history")
async def get_recalibration_history(
    profile_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
    hypothesis_id: str | None = None,
    ...
    return latest_recalibrations(db, profile_id, hypothesis_id)
...
@router.post("/career-experiment-sessions/{session_id}/roadmap")
async def confirm_profile_experiment_roadmap(
    session_id: str,
    payload: RoadmapConfirmationRequest,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> dict[str, Any]:
    session = require_session(db, session_id, user)
    ...
    try:
        return confirm_experiment_roadmap(db, profile, session, payload.confirmed)
    except Exception as error:
        _handle(error)
        raise
