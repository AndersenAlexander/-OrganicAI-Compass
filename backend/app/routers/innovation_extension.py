...
class JournalRequest(DictRequest):
    ...
    system_suggestions: list[dict[str, Any]] = Field(default_factory=list)
    ...
    adviser_inputs: list[dict[str, Any]] = Field(default_factory=list)
    user_reasoning: str = ""
...
@router.delete("/profiles/{profile_id}/job-captures/{capture_id}")
async def delete_job_capture(
    profile_id: str,
    capture_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
    ...
    profile = require_profile(db, profile_id, user)
    try:
        return archive_job_capture(db, profile, capture_id, user.id if user else profile.user_id)
    except Exception as error:
        _handle(error)
        raise
...
async def get_careers(
    ...
    return list_career_roles(db, family, search=search, skill=skill)
