...
from app.services.career_resilience_engine import list_profile_evidence_gaps
...
async def get_profile_evidence_gaps(
    ...
    profile = require_profile(db, profile_id, user)
    try:
        adaptive = discover_evidence_gaps(db, profile)
        structured = list_profile_evidence_gaps(db, profile)
        adaptive["career_hypothesis_gaps"] = structured.get("gaps", [])
        adaptive["career_hypothesis_gap_summary"] = structured.get("summary", {})
        adaptive["explanation_rule"] = {
            "why": "The gap links a current Career Hypothesis to a capability or requirement that is not yet sufficiently supported.",
            "source": ["CAREER_HYPOTHESIS", "EVIDENCE", "SYSTEM_DERIVED"],
            "missing": "Missing evidence is not interpreted as inability.",
            "can_change": "A bounded experiment, practical artefact, verification, or corrected evidence can change the gap status.",
            "does_not_mean": "It is not a prediction of low fit or employment outcome.",
        }
        return adaptive
    except Exception as error:
        _handle(error)
...
@router.get("/profiles/{profile_id}/adaptive-experiment-runs")
async def get_adaptive_profile_runs(
    profile_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[User | None, Depends(get_optional_user)],
) -> list[dict[str, Any]]:
    profile = require_profile(db, profile_id, user)
    return list_adaptive_runs(db, profile)
