...
def test_demo_restore_removes_career_experiment_children_before_roadmap_actions(monkeypatch):
    db = session(); user, profile, _ = seed(monkeypatch, db)
    assert db.scalar(select(func.count()).select_from(CareerExperimentSession).where(CareerExperimentSession.profile_id == profile.id, CareerExperimentSession.roadmap_action_id.is_not(None))) >= 1
    _, restored, _ = demo_seed_service.restore_demo(db)
    assert db.get(User, user.id).is_demo
    assert db.scalar(select(func.count()).select_from(CareerExperimentSession).where(CareerExperimentSession.profile_id == restored.id)) >= 2
...
def test_research_exports_exclude_demo_by_default_and_allow_explicit_filter(monkeypatch):
    ...
    monkeypatch.setattr(research, "research_readiness", lambda: {"ready": True})
