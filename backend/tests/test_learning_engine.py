...
def test_learning_recommendations_rank_resources_and_store_factors():
    db = session()
    _, item, match = complete_demo_assessment(db)
    db.add(CareerHypothesis(profile_id=item.id, user_id=item.user_id, career_match_id=match.id, title=match.title, status="active", current_version_number=1))
    db.commit()
    ...
    assert all(rec["career_direction"]["label"] == match.title for rec in result["recommendations"])
    assert all(rec["career_direction"]["source"] == "Career Hypothesis" for rec in result["recommendations"])
