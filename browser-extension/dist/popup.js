...
async function submit(action) {
    ...
    if (!rawProfileId)
        throw new Error("Profile ID is required. Copy it from the Browser Extension settings page in OrganicAI Compass.");
    if (!token)
...
function messageForStatus(status) {
    if (status === 401)
        return "Login or a valid extension connection is required.";
    if (status === 403)
        return "Capture was rejected. Check that the token belongs to this profile and has not expired.";
    if (status === 404)
        return "Profile or extension connection was not found.";
    if (status === 422)
        return "Capture could not be saved. Review the URL, selected text, and connection details.";
...
async function initialise() {
    ...
    getInput("saveButton").addEventListener("click", () => submit("save").catch((error) => setStatus(error.message)));
    getInput("saveAnalyseButton").addEventListener("click", () => submit("save_and_analyse").catch((error) => setStatus(error.message)));
}
